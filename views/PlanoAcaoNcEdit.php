...
?>
<main class="edit">
<form name="fplano_acao_ncedit" id="fplano_acao_ncedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { plano_acao_nc: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fplano_acao_ncedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fplano_acao_ncedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idplano_acao_nc", [fields.idplano_acao_nc.visible && fields.idplano_acao_nc.required ? ew.Validators.required(fields.idplano_acao_nc.caption) : null], fields.idplano_acao_nc.isInvalid],
...
<div class="ew-nav<?= $Page->MultiPages->containerClasses() ?>" id="pages_PlanoAcaoNcEdit"><!-- multi-page tabs -->
...
<?php if ($Page->idplano_acao_nc->Visible) { // idplano_acao_nc ?>
    <div id="r_idplano_acao_nc"<?= $Page->idplano_acao_nc->rowAttributes() ?>>
        <label id="elh_plano_acao_nc_idplano_acao_nc" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idplano_acao_nc->caption() ?><?= $Page->idplano_acao_nc->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idplano_acao_nc->cellAttributes() ?>>
<span id="el_plano_acao_nc_idplano_acao_nc">
<span<?= $Page->idplano_acao_nc->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idplano_acao_nc->getDisplayValue($Page->idplano_acao_nc->EditValue))) ?>"></span>
<input type="hidden" data-table="plano_acao_nc" data-field="x_idplano_acao_nc" data-hidden="1" data-page="1" name="x_idplano_acao_nc" id="x_idplano_acao_nc" value="<?= HtmlEncode($Page->idplano_acao_nc->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_plano_acao_nc_nao_conformidade_idnao_conformidade">
    <select
        ...
        data-select2-id="fplano_acao_ncedit_x_nao_conformidade_idnao_conformidade"
        <?php } ?>
...
<?php if (!$Page->nao_conformidade_idnao_conformidade->IsNativeSelect) { ?>
<script>
loadjs.ready("fplano_acao_ncedit", function() {
    var options = { name: "x_nao_conformidade_idnao_conformidade", selectId: "fplano_acao_ncedit_x_nao_conformidade_idnao_conformidade" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fplano_acao_ncedit.lists.nao_conformidade_idnao_conformidade?.lookupOptions.length) {
        options.data = { id: "x_nao_conformidade_idnao_conformidade", form: "fplano_acao_ncedit" };
    } else {
        options.ajax = { id: "x_nao_conformidade_idnao_conformidade", form: "fplano_acao_ncedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_plano_acao_nc_usuario_idusuario">
    <select
        ...
        data-select2-id="fplano_acao_ncedit_x_usuario_idusuario"
        data-table="plano_acao_nc"
...
<script>
loadjs.ready("fplano_acao_ncedit", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fplano_acao_ncedit_x_usuario_idusuario" };
    if (fplano_acao_ncedit.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fplano_acao_ncedit" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fplano_acao_ncedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fplano_acao_ncedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fplano_acao_ncedit", "x_dt_limite", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fplano_acao_ncedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fplano_acao_ncedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

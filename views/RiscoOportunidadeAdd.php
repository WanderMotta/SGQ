...
$Page->showMessage();
?>
<form name="frisco_oportunidadeadd" id="frisco_oportunidadeadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_risco_oportunidade_origem_risco_oportunidade_idorigem_risco_oportunidade">
    <select
        ...
        data-select2-id="frisco_oportunidadeadd_x_origem_risco_oportunidade_idorigem_risco_oportunidade"
        <?php } ?>
        data-table="risco_oportunidade"
...
<?php if (!$Page->origem_risco_oportunidade_idorigem_risco_oportunidade->IsNativeSelect) { ?>
<script>
loadjs.ready("frisco_oportunidadeadd", function() {
    var options = { name: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", selectId: "frisco_oportunidadeadd_x_origem_risco_oportunidade_idorigem_risco_oportunidade" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frisco_oportunidadeadd.lists.origem_risco_oportunidade_idorigem_risco_oportunidade?.lookupOptions.length) {
        options.data = { id: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", form: "frisco_oportunidadeadd" };
    } else {
        options.ajax = { id: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", form: "frisco_oportunidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_risco_oportunidade_frequencia_idfrequencia">
    <select
        ...
        data-select2-id="frisco_oportunidadeadd_x_frequencia_idfrequencia"
        <?php } ?>
        data-table="risco_oportunidade"
...
<script>
loadjs.ready("frisco_oportunidadeadd", function() {
    var options = { name: "x_frequencia_idfrequencia", selectId: "frisco_oportunidadeadd_x_frequencia_idfrequencia" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frisco_oportunidadeadd.lists.frequencia_idfrequencia?.lookupOptions.length) {
        options.data = { id: "x_frequencia_idfrequencia", form: "frisco_oportunidadeadd" };
    } else {
        options.ajax = { id: "x_frequencia_idfrequencia", form: "frisco_oportunidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_risco_oportunidade_impacto_idimpacto">
    <select
        ...
        data-select2-id="frisco_oportunidadeadd_x_impacto_idimpacto"
        <?php } ?>
        data-table="risco_oportunidade"
...
<script>
loadjs.ready("frisco_oportunidadeadd", function() {
    var options = { name: "x_impacto_idimpacto", selectId: "frisco_oportunidadeadd_x_impacto_idimpacto" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frisco_oportunidadeadd.lists.impacto_idimpacto?.lookupOptions.length) {
        options.data = { id: "x_impacto_idimpacto", form: "frisco_oportunidadeadd" };
    } else {
        options.ajax = { id: "x_impacto_idimpacto", form: "frisco_oportunidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_risco_oportunidade_acao_risco_oportunidade_idacao_risco_oportunidade">
    <select
        ...
        data-select2-id="frisco_oportunidadeadd_x_acao_risco_oportunidade_idacao_risco_oportunidade"
        <?php } ?>
        data-table="risco_oportunidade"
...
<script>
loadjs.ready("frisco_oportunidadeadd", function() {
    var options = { name: "x_acao_risco_oportunidade_idacao_risco_oportunidade", selectId: "frisco_oportunidadeadd_x_acao_risco_oportunidade_idacao_risco_oportunidade" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frisco_oportunidadeadd.lists.acao_risco_oportunidade_idacao_risco_oportunidade?.lookupOptions.length) {
        options.data = { id: "x_acao_risco_oportunidade_idacao_risco_oportunidade", form: "frisco_oportunidadeadd" };
    } else {
        options.ajax = { id: "x_acao_risco_oportunidade_idacao_risco_oportunidade", form: "frisco_oportunidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("plano_acao", explode(",", $Page->getCurrentDetailTable())) && $plano_acao->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="frisco_oportunidadeadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="frisco_oportunidadeadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

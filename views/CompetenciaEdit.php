...
?>
<main class="edit">
<form name="fcompetenciaedit" id="fcompetenciaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { competencia: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fcompetenciaedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fcompetenciaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idcompetencia", [fields.idcompetencia.visible && fields.idcompetencia.required ? ew.Validators.required(fields.idcompetencia.caption) : null], fields.idcompetencia.isInvalid],
...
<?php if ($Page->idcompetencia->Visible) { // idcompetencia ?>
    <div id="r_idcompetencia"<?= $Page->idcompetencia->rowAttributes() ?>>
        <label id="elh_competencia_idcompetencia" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idcompetencia->caption() ?><?= $Page->idcompetencia->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idcompetencia->cellAttributes() ?>>
<span id="el_competencia_idcompetencia">
<span<?= $Page->idcompetencia->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idcompetencia->getDisplayValue($Page->idcompetencia->EditValue))) ?>"></span>
<input type="hidden" data-table="competencia" data-field="x_idcompetencia" data-hidden="1" name="x_idcompetencia" id="x_idcompetencia" value="<?= HtmlEncode($Page->idcompetencia->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fcompetenciaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcompetenciaedit", "x_data_base", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcompetenciaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcompetenciaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

...
$Page->showMessage();
?>
<form name="fnao_conformidadeadd" id="fnao_conformidadeadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready(["fnao_conformidadeadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fnao_conformidadeadd", "x_dt_ocorrencia", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_nao_conformidade_processo_idprocesso">
    <select
        ...
        data-select2-id="fnao_conformidadeadd_x_processo_idprocesso"
        data-table="nao_conformidade"
...
<script>
loadjs.ready("fnao_conformidadeadd", function() {
    var options = { name: "x_processo_idprocesso", selectId: "fnao_conformidadeadd_x_processo_idprocesso" };
    if (fnao_conformidadeadd.lists.processo_idprocesso?.lookupOptions.length) {
        options.data = { id: "x_processo_idprocesso", form: "fnao_conformidadeadd" };
    } else {
        options.ajax = { id: "x_processo_idprocesso", form: "fnao_conformidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_nao_conformidade_origem_risco_oportunidade_idorigem_risco_oportunidade">
    <select
        ...
        data-select2-id="fnao_conformidadeadd_x_origem_risco_oportunidade_idorigem_risco_oportunidade"
        <?php } ?>
        data-table="nao_conformidade"
...
<script>
loadjs.ready("fnao_conformidadeadd", function() {
    var options = { name: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", selectId: "fnao_conformidadeadd_x_origem_risco_oportunidade_idorigem_risco_oportunidade" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fnao_conformidadeadd.lists.origem_risco_oportunidade_idorigem_risco_oportunidade?.lookupOptions.length) {
        options.data = { id: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", form: "fnao_conformidadeadd" };
    } else {
        options.ajax = { id: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", form: "fnao_conformidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_nao_conformidade_departamentos_iddepartamentos">
    <select
        ...
        data-select2-id="fnao_conformidadeadd_x_departamentos_iddepartamentos"
        <?php } ?>
        data-table="nao_conformidade"
...
<script>
loadjs.ready("fnao_conformidadeadd", function() {
    var options = { name: "x_departamentos_iddepartamentos", selectId: "fnao_conformidadeadd_x_departamentos_iddepartamentos" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fnao_conformidadeadd.lists.departamentos_iddepartamentos?.lookupOptions.length) {
        options.data = { id: "x_departamentos_iddepartamentos", form: "fnao_conformidadeadd" };
    } else {
        options.ajax = { id: "x_departamentos_iddepartamentos", form: "fnao_conformidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("plano_acao_nc", explode(",", $Page->getCurrentDetailTable())) && $plano_acao_nc->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fnao_conformidadeadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fnao_conformidadeadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

...
?>
<main class="edit">
<form name="fdocumento_registroedit" id="fdocumento_registroedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { documento_registro: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fdocumento_registroedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdocumento_registroedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["iddocumento_registro", [fields.iddocumento_registro.visible && fields.iddocumento_registro.required ? ew.Validators.required(fields.iddocumento_registro.caption) : null], fields.iddocumento_registro.isInvalid],
...
<?php if ($Page->iddocumento_registro->Visible) { // iddocumento_registro ?>
    <div id="r_iddocumento_registro"<?= $Page->iddocumento_registro->rowAttributes() ?>>
        <label id="elh_documento_registro_iddocumento_registro" class="<?= $Page->LeftColumnClass ?>"><?= $Page->iddocumento_registro->caption() ?><?= $Page->iddocumento_registro->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->iddocumento_registro->cellAttributes() ?>>
<span id="el_documento_registro_iddocumento_registro">
<span<?= $Page->iddocumento_registro->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->iddocumento_registro->getDisplayValue($Page->iddocumento_registro->EditValue))) ?>"></span>
<input type="hidden" data-table="documento_registro" data-field="x_iddocumento_registro" data-hidden="1" name="x_iddocumento_registro" id="x_iddocumento_registro" value="<?= HtmlEncode($Page->iddocumento_registro->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fdocumento_registroedit_x_categoria_documento_idcategoria_documento"
...
<script>
loadjs.ready("fdocumento_registroedit", function() {
    var options = { name: "x_categoria_documento_idcategoria_documento", selectId: "fdocumento_registroedit_x_categoria_documento_idcategoria_documento" };
    if (fdocumento_registroedit.lists.categoria_documento_idcategoria_documento?.lookupOptions.length) {
        options.data = { id: "x_categoria_documento_idcategoria_documento", form: "fdocumento_registroedit" };
    } else {
        options.ajax = { id: "x_categoria_documento_idcategoria_documento", form: "fdocumento_registroedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_documento_registro_usuario_idusuario">
    <select
        ...
        data-select2-id="fdocumento_registroedit_x_usuario_idusuario"
        <?php } ?>
        data-table="documento_registro"
...
<script>
loadjs.ready("fdocumento_registroedit", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fdocumento_registroedit_x_usuario_idusuario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdocumento_registroedit.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fdocumento_registroedit" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fdocumento_registroedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdocumento_registroedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdocumento_registroedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

...
$Page->showMessage();
?>
<form name="fplano_auditoria_intadd" id="fplano_auditoria_intadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready(["fplano_auditoria_intadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fplano_auditoria_intadd", "x_data", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_plano_auditoria_int_usuario_idusuario">
    <select
        ...
        data-select2-id="fplano_auditoria_intadd_x_usuario_idusuario"
...
<script>
loadjs.ready("fplano_auditoria_intadd", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fplano_auditoria_intadd_x_usuario_idusuario" };
    if (fplano_auditoria_intadd.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fplano_auditoria_intadd" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fplano_auditoria_intadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_arquivo" id= "fa_x_arquivo" value="0">
...
<?php
    if (in_array("item_plano_aud_int", explode(",", $Page->getCurrentDetailTable())) && $item_plano_aud_int->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fplano_auditoria_intadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fplano_auditoria_intadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

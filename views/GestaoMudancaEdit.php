...
?>
<main class="edit">
<form name="fgestao_mudancaedit" id="fgestao_mudancaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { gestao_mudanca: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fgestao_mudancaedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fgestao_mudancaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idgestao_mudanca", [fields.idgestao_mudanca.visible && fields.idgestao_mudanca.required ? ew.Validators.required(fields.idgestao_mudanca.caption) : null], fields.idgestao_mudanca.isInvalid],
...
<?php if ($Page->idgestao_mudanca->Visible) { // idgestao_mudanca ?>
    <div id="r_idgestao_mudanca"<?= $Page->idgestao_mudanca->rowAttributes() ?>>
        <label id="elh_gestao_mudanca_idgestao_mudanca" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idgestao_mudanca->caption() ?><?= $Page->idgestao_mudanca->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idgestao_mudanca->cellAttributes() ?>>
<span id="el_gestao_mudanca_idgestao_mudanca">
<span<?= $Page->idgestao_mudanca->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idgestao_mudanca->getDisplayValue($Page->idgestao_mudanca->EditValue))) ?>"></span>
<input type="hidden" data-table="gestao_mudanca" data-field="x_idgestao_mudanca" data-hidden="1" name="x_idgestao_mudanca" id="x_idgestao_mudanca" value="<?= HtmlEncode($Page->idgestao_mudanca->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fgestao_mudancaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fgestao_mudancaedit", "x_dt_inicio", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fgestao_mudancaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fgestao_mudancaedit", "x_prazo_ate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_gestao_mudanca_departamentos_iddepartamentos">
    <select
        ...
        data-select2-id="fgestao_mudancaedit_x_departamentos_iddepartamentos"
...
<script>
loadjs.ready("fgestao_mudancaedit", function() {
    var options = { name: "x_departamentos_iddepartamentos", selectId: "fgestao_mudancaedit_x_departamentos_iddepartamentos" };
    if (fgestao_mudancaedit.lists.departamentos_iddepartamentos?.lookupOptions.length) {
        options.data = { id: "x_departamentos_iddepartamentos", form: "fgestao_mudancaedit" };
    } else {
        options.ajax = { id: "x_departamentos_iddepartamentos", form: "fgestao_mudancaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_gestao_mudanca_usuario_idusuario">
    <select
        ...
        data-select2-id="fgestao_mudancaedit_x_usuario_idusuario"
        <?php } ?>
        data-table="gestao_mudanca"
...
<script>
loadjs.ready("fgestao_mudancaedit", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fgestao_mudancaedit_x_usuario_idusuario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fgestao_mudancaedit.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fgestao_mudancaedit" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fgestao_mudancaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fgestao_mudancaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fgestao_mudancaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

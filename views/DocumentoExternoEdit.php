...
?>
<main class="edit">
<form name="fdocumento_externoedit" id="fdocumento_externoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { documento_externo: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fdocumento_externoedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdocumento_externoedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["iddocumento_externo", [fields.iddocumento_externo.visible && fields.iddocumento_externo.required ? ew.Validators.required(fields.iddocumento_externo.caption) : null], fields.iddocumento_externo.isInvalid],
...
<?php if ($Page->iddocumento_externo->Visible) { // iddocumento_externo ?>
    <div id="r_iddocumento_externo"<?= $Page->iddocumento_externo->rowAttributes() ?>>
        <label id="elh_documento_externo_iddocumento_externo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->iddocumento_externo->caption() ?><?= $Page->iddocumento_externo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->iddocumento_externo->cellAttributes() ?>>
<span id="el_documento_externo_iddocumento_externo">
<span<?= $Page->iddocumento_externo->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->iddocumento_externo->getDisplayValue($Page->iddocumento_externo->EditValue))) ?>"></span>
<input type="hidden" data-table="documento_externo" data-field="x_iddocumento_externo" data-hidden="1" name="x_iddocumento_externo" id="x_iddocumento_externo" value="<?= HtmlEncode($Page->iddocumento_externo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fdocumento_externoedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdocumento_externoedit", "x_valido_ate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_documento_externo_usuario_responsavel">
    <select
        ...
        data-select2-id="fdocumento_externoedit_x_usuario_responsavel"
        data-table="documento_externo"
...
<script>
loadjs.ready("fdocumento_externoedit", function() {
    var options = { name: "x_usuario_responsavel", selectId: "fdocumento_externoedit_x_usuario_responsavel" };
    if (fdocumento_externoedit.lists.usuario_responsavel?.lookupOptions.length) {
        options.data = { id: "x_usuario_responsavel", form: "fdocumento_externoedit" };
    } else {
        options.ajax = { id: "x_usuario_responsavel", form: "fdocumento_externoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdocumento_externoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdocumento_externoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

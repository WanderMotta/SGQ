...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fimpactosrch" id="fimpactosrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
<div id="fimpactosrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { impacto: currentTable } });
var currentForm;
var fimpactosrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
...
<input type="hidden" name="z_tipo_risco_oportunidade_idtipo_risco_oportunidade" id="z_tipo_risco_oportunidade_idtipo_risco_oportunidade" value="=">
        ...
        <div id="el_impacto_tipo_risco_oportunidade_idtipo_risco_oportunidade" class="ew-search-field">
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="fimpactosrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="fimpactosrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="fimpactosrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="fimpactosrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_impacto" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_impactolist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
?>
<?php if ($Page->tipo_risco_oportunidade_idtipo_risco_oportunidade->Visible) { // tipo_risco_oportunidade_idtipo_risco_oportunidade ?>
        <th data-name="tipo_risco_oportunidade_idtipo_risco_oportunidade" class="<?= $Page->tipo_risco_oportunidade_idtipo_risco_oportunidade->headerCellClass() ?>"><div id="elh_impacto_tipo_risco_oportunidade_idtipo_risco_oportunidade" class="impacto_tipo_risco_oportunidade_idtipo_risco_oportunidade"><?= $Page->renderFieldHeader($Page->tipo_risco_oportunidade_idtipo_risco_oportunidade) ?></div></th>
<?php } ?>
<?php if ($Page->impacto->Visible) { // impacto ?>
        <th data-name="impacto" class="<?= $Page->impacto->headerCellClass() ?>"><div id="elh_impacto_impacto" class="impacto_impacto"><?= $Page->renderFieldHeader($Page->impacto) ?></div></th>
<?php } ?>
<?php if ($Page->grau->Visible) { // grau ?>
        <th data-name="grau" class="<?= $Page->grau->headerCellClass() ?>"><div id="elh_impacto_grau" class="impacto_grau"><?= $Page->renderFieldHeader($Page->grau) ?></div></th>
<?php } ?>
<?php if ($Page->obs->Visible) { // obs ?>
        <th data-name="obs" class="<?= $Page->obs->headerCellClass() ?>"><div id="elh_impacto_obs" class="impacto_obs"><?= $Page->renderFieldHeader($Page->obs) ?></div></th>
<?php } ?>
<?php
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_impacto_tipo_risco_oportunidade_idtipo_risco_oportunidade" class="el_impacto_tipo_risco_oportunidade_idtipo_risco_oportunidade">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_impacto_impacto" class="el_impacto_impacto">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_impacto_grau" class="el_impacto_grau">
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_impacto_obs" class="el_impacto_obs">

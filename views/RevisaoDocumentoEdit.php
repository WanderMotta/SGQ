...
?>
<main class="edit">
<form name="frevisao_documentoedit" id="frevisao_documentoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { revisao_documento: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var frevisao_documentoedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("frevisao_documentoedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idrevisao_documento", [fields.idrevisao_documento.visible && fields.idrevisao_documento.required ? ew.Validators.required(fields.idrevisao_documento.caption) : null], fields.idrevisao_documento.isInvalid],
...
<?php if ($Page->idrevisao_documento->Visible) { // idrevisao_documento ?>
    <div id="r_idrevisao_documento"<?= $Page->idrevisao_documento->rowAttributes() ?>>
        <label id="elh_revisao_documento_idrevisao_documento" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idrevisao_documento->caption() ?><?= $Page->idrevisao_documento->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idrevisao_documento->cellAttributes() ?>>
<span id="el_revisao_documento_idrevisao_documento">
<span<?= $Page->idrevisao_documento->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idrevisao_documento->getDisplayValue($Page->idrevisao_documento->EditValue))) ?>"></span>
<input type="hidden" data-table="revisao_documento" data-field="x_idrevisao_documento" data-hidden="1" name="x_idrevisao_documento" id="x_idrevisao_documento" value="<?= HtmlEncode($Page->idrevisao_documento->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_revisao_documento_documento_interno_iddocumento_interno">
    <select
        ...
        data-select2-id="frevisao_documentoedit_x_documento_interno_iddocumento_interno"
        <?php } ?>
...
<?php if (!$Page->documento_interno_iddocumento_interno->IsNativeSelect) { ?>
<script>
loadjs.ready("frevisao_documentoedit", function() {
    var options = { name: "x_documento_interno_iddocumento_interno", selectId: "frevisao_documentoedit_x_documento_interno_iddocumento_interno" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frevisao_documentoedit.lists.documento_interno_iddocumento_interno?.lookupOptions.length) {
        options.data = { id: "x_documento_interno_iddocumento_interno", form: "frevisao_documentoedit" };
    } else {
        options.ajax = { id: "x_documento_interno_iddocumento_interno", form: "frevisao_documentoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["frevisao_documentoedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("frevisao_documentoedit", "x_dt_cadastro", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->status_documento_idstatus_documento->Visible) { // status_documento_idstatus_documento ?>
    <div id="r_status_documento_idstatus_documento"<?= $Page->status_documento_idstatus_documento->rowAttributes() ?>>
        <label id="elh_revisao_documento_status_documento_idstatus_documento" class="<?= $Page->LeftColumnClass ?>"><?= $Page->status_documento_idstatus_documento->caption() ?><?= $Page->status_documento_idstatus_documento->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->status_documento_idstatus_documento->cellAttributes() ?>>
...
<span class="form-control-plaintext"><?= $Page->status_documento_idstatus_documento->getDisplayValue($Page->status_documento_idstatus_documento->EditValue) ?></span></span>
<input type="hidden" data-table="revisao_documento" data-field="x_status_documento_idstatus_documento" data-hidden="1" name="x_status_documento_idstatus_documento" id="x_status_documento_idstatus_documento" value="<?= HtmlEncode($Page->status_documento_idstatus_documento->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_revisao_documento_usuario_elaborador">
    <select
        ...
        data-select2-id="frevisao_documentoedit_x_usuario_elaborador"
        <?php } ?>
        data-table="revisao_documento"
...
<script>
loadjs.ready("frevisao_documentoedit", function() {
    var options = { name: "x_usuario_elaborador", selectId: "frevisao_documentoedit_x_usuario_elaborador" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frevisao_documentoedit.lists.usuario_elaborador?.lookupOptions.length) {
        options.data = { id: "x_usuario_elaborador", form: "frevisao_documentoedit" };
    } else {
        options.ajax = { id: "x_usuario_elaborador", form: "frevisao_documentoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_revisao_documento_usuario_aprovador">
    <select
        ...
        data-select2-id="frevisao_documentoedit_x_usuario_aprovador"
        <?php } ?>
        data-table="revisao_documento"
...
<script>
loadjs.ready("frevisao_documentoedit", function() {
    var options = { name: "x_usuario_aprovador", selectId: "frevisao_documentoedit_x_usuario_aprovador" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frevisao_documentoedit.lists.usuario_aprovador?.lookupOptions.length) {
        options.data = { id: "x_usuario_aprovador", form: "frevisao_documentoedit" };
    } else {
        options.ajax = { id: "x_usuario_aprovador", form: "frevisao_documentoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="frevisao_documentoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="frevisao_documentoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

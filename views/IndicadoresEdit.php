...
?>
<main class="edit">
<form name="findicadoresedit" id="findicadoresedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { indicadores: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var findicadoresedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("findicadoresedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idindicadores", [fields.idindicadores.visible && fields.idindicadores.required ? ew.Validators.required(fields.idindicadores.caption) : null], fields.idindicadores.isInvalid],
...
<?php if ($Page->idindicadores->Visible) { // idindicadores ?>
    <div id="r_idindicadores"<?= $Page->idindicadores->rowAttributes() ?>>
        <label id="elh_indicadores_idindicadores" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idindicadores->caption() ?><?= $Page->idindicadores->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idindicadores->cellAttributes() ?>>
<span id="el_indicadores_idindicadores">
<span<?= $Page->idindicadores->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idindicadores->getDisplayValue($Page->idindicadores->EditValue))) ?>"></span>
<input type="hidden" data-table="indicadores" data-field="x_idindicadores" data-hidden="1" name="x_idindicadores" id="x_idindicadores" value="<?= HtmlEncode($Page->idindicadores->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php
    if (in_array("graficos", explode(",", $Page->getCurrentDetailTable())) && $graficos->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="findicadoresedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="findicadoresedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

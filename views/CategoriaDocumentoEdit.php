...
?>
<main class="edit">
<form name="fcategoria_documentoedit" id="fcategoria_documentoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { categoria_documento: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fcategoria_documentoedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fcategoria_documentoedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idcategoria_documento", [fields.idcategoria_documento.visible && fields.idcategoria_documento.required ? ew.Validators.required(fields.idcategoria_documento.caption) : null], fields.idcategoria_documento.isInvalid],
...
<?php if ($Page->idcategoria_documento->Visible) { // idcategoria_documento ?>
    <div id="r_idcategoria_documento"<?= $Page->idcategoria_documento->rowAttributes() ?>>
        <label id="elh_categoria_documento_idcategoria_documento" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idcategoria_documento->caption() ?><?= $Page->idcategoria_documento->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idcategoria_documento->cellAttributes() ?>>
<span id="el_categoria_documento_idcategoria_documento">
<span<?= $Page->idcategoria_documento->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idcategoria_documento->getDisplayValue($Page->idcategoria_documento->EditValue))) ?>"></span>
<input type="hidden" data-table="categoria_documento" data-field="x_idcategoria_documento" data-hidden="1" name="x_idcategoria_documento" id="x_idcategoria_documento" value="<?= HtmlEncode($Page->idcategoria_documento->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcategoria_documentoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcategoria_documentoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

...
$Page->showMessage();
?>
<form name="fplano_acaoadd" id="fplano_acaoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_plano_acao_risco_oportunidade_idrisco_oportunidade">
    <select
        ...
        data-select2-id="fplano_acaoadd_x_risco_oportunidade_idrisco_oportunidade"
        <?php } ?>
...
<?php if (!$Page->risco_oportunidade_idrisco_oportunidade->IsNativeSelect) { ?>
<script>
loadjs.ready("fplano_acaoadd", function() {
    var options = { name: "x_risco_oportunidade_idrisco_oportunidade", selectId: "fplano_acaoadd_x_risco_oportunidade_idrisco_oportunidade" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fplano_acaoadd.lists.risco_oportunidade_idrisco_oportunidade?.lookupOptions.length) {
        options.data = { id: "x_risco_oportunidade_idrisco_oportunidade", form: "fplano_acaoadd" };
    } else {
        options.ajax = { id: "x_risco_oportunidade_idrisco_oportunidade", form: "fplano_acaoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_plano_acao_departamentos_iddepartamentos">
    <select
        ...
        data-select2-id="fplano_acaoadd_x_departamentos_iddepartamentos"
        <?php } ?>
        data-table="plano_acao"
...
<script>
loadjs.ready("fplano_acaoadd", function() {
    var options = { name: "x_departamentos_iddepartamentos", selectId: "fplano_acaoadd_x_departamentos_iddepartamentos" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fplano_acaoadd.lists.departamentos_iddepartamentos?.lookupOptions.length) {
        options.data = { id: "x_departamentos_iddepartamentos", form: "fplano_acaoadd" };
    } else {
        options.ajax = { id: "x_departamentos_iddepartamentos", form: "fplano_acaoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_plano_acao_origem_risco_oportunidade_idorigem_risco_oportunidade">
    <select
        ...
        data-select2-id="fplano_acaoadd_x_origem_risco_oportunidade_idorigem_risco_oportunidade"
        <?php } ?>
        data-table="plano_acao"
...
<script>
loadjs.ready("fplano_acaoadd", function() {
    var options = { name: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", selectId: "fplano_acaoadd_x_origem_risco_oportunidade_idorigem_risco_oportunidade" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fplano_acaoadd.lists.origem_risco_oportunidade_idorigem_risco_oportunidade?.lookupOptions.length) {
        options.data = { id: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", form: "fplano_acaoadd" };
    } else {
        options.ajax = { id: "x_origem_risco_oportunidade_idorigem_risco_oportunidade", form: "fplano_acaoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fplano_acaoadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fplano_acaoadd", "x_dt_limite", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fplano_acaoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fplano_acaoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

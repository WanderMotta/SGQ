...
$Page->showMessage();
?>
<form name="fdocumento_internoadd" id="fdocumento_internoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fdocumento_internoadd_x_categoria_documento_idcategoria_documento"
        data-table="documento_interno"
...
<script>
loadjs.ready("fdocumento_internoadd", function() {
    var options = { name: "x_categoria_documento_idcategoria_documento", selectId: "fdocumento_internoadd_x_categoria_documento_idcategoria_documento" };
    if (fdocumento_internoadd.lists.categoria_documento_idcategoria_documento?.lookupOptions.length) {
        options.data = { id: "x_categoria_documento_idcategoria_documento", form: "fdocumento_internoadd" };
    } else {
        options.ajax = { id: "x_categoria_documento_idcategoria_documento", form: "fdocumento_internoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_documento_interno_processo_idprocesso">
    <select
        ...
        data-select2-id="fdocumento_internoadd_x_processo_idprocesso"
        <?php } ?>
        data-table="documento_interno"
...
<script>
loadjs.ready("fdocumento_internoadd", function() {
    var options = { name: "x_processo_idprocesso", selectId: "fdocumento_internoadd_x_processo_idprocesso" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdocumento_internoadd.lists.processo_idprocesso?.lookupOptions.length) {
        options.data = { id: "x_processo_idprocesso", form: "fdocumento_internoadd" };
    } else {
        options.ajax = { id: "x_processo_idprocesso", form: "fdocumento_internoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_documento_interno_usuario_idusuario">
    <select
        ...
        data-select2-id="fdocumento_internoadd_x_usuario_idusuario"
        data-table="documento_interno"
...
<script>
loadjs.ready("fdocumento_internoadd", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fdocumento_internoadd_x_usuario_idusuario" };
    if (fdocumento_internoadd.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fdocumento_internoadd" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fdocumento_internoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("revisao_documento", explode(",", $Page->getCurrentDetailTable())) && $revisao_documento->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdocumento_internoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdocumento_internoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

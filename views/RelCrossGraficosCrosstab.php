<?php namespace PHPMaker2023\sgq; ?>
<?php

namespace PHPMaker2023\sgq;

// Page object
$RelCrossGraficosCrosstab = &$Page;
?>
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { rel_cross_graficos: currentTable } });
...
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<form name="frel_cross_graficossrch" id="frel_cross_graficossrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="on">
<div id="frel_cross_graficossrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { rel_cross_graficos: currentTable } });
var currentPageID = ew.PAGE_ID = "crosstab";
var currentForm;
var frel_cross_graficossrch, currentSearchForm, currentAdvancedSearchForm;
...
?>
    <div id="xs_indicadores_idindicadores" class="col-sm-auto d-sm-flex align-items-start mb-3 px-0 pe-sm-2<?= $Page->indicadores_idindicadores->UseFilter ? " ew-filter-field" : "" ?>">
        ...
        <div id="el_rel_cross_graficos_indicadores_idindicadores" class="ew-search-field">
    <select
        ...
        data-select2-id="frel_cross_graficossrch_x_indicadores_idindicadores"
        <?php } ?>
        data-table="rel_cross_graficos"
...
<?php if (!$Page->indicadores_idindicadores->IsNativeSelect) { ?>
<script>
loadjs.ready("frel_cross_graficossrch", function() {
    var options = { name: "x_indicadores_idindicadores", selectId: "frel_cross_graficossrch_x_indicadores_idindicadores" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (frel_cross_graficossrch.lists.indicadores_idindicadores?.lookupOptions.length) {
        options.data = { id: "x_indicadores_idindicadores", form: "frel_cross_graficossrch" };
    } else {
        options.ajax = { id: "x_indicadores_idindicadores", form: "frel_cross_graficossrch", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.rel_cross_graficos.fields.indicadores_idindicadores.selectOptions);
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_rel_cross_graficos" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>">

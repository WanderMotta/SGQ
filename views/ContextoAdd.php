...
$Page->showMessage();
?>
<form name="fcontextoadd" id="fcontextoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready("fcontextoadd", function() {
    fcontextoadd.createAutoSuggest(Object.assign({"id":"x_revisao","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->revisao->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.contexto.fields.revisao.autoSuggestOptions));
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fcontextoadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcontextoadd", "x_data", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_contexto_usuario_idusuario">
    <select
        ...
        data-select2-id="fcontextoadd_x_usuario_idusuario"
        <?php } ?>
        data-table="contexto"
...
<script>
loadjs.ready("fcontextoadd", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fcontextoadd_x_usuario_idusuario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcontextoadd.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fcontextoadd" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fcontextoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_contexto_usuario_idusuario1">
    <select
        ...
        data-select2-id="fcontextoadd_x_usuario_idusuario1"
        <?php } ?>
        data-table="contexto"
...
<script>
loadjs.ready("fcontextoadd", function() {
    var options = { name: "x_usuario_idusuario1", selectId: "fcontextoadd_x_usuario_idusuario1" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcontextoadd.lists.usuario_idusuario1?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario1", form: "fcontextoadd" };
    } else {
        options.ajax = { id: "x_usuario_idusuario1", form: "fcontextoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("analise_swot", explode(",", $Page->getCurrentDetailTable())) && $analise_swot->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcontextoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcontextoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

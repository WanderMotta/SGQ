...
?>
<main class="edit">
<form name="fanalise_swotedit" id="fanalise_swotedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { analise_swot: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fanalise_swotedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fanalise_swotedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idanalise_swot", [fields.idanalise_swot.visible && fields.idanalise_swot.required ? ew.Validators.required(fields.idanalise_swot.caption) : null], fields.idanalise_swot.isInvalid],
...
<?php if ($Page->idanalise_swot->Visible) { // idanalise_swot ?>
    <div id="r_idanalise_swot"<?= $Page->idanalise_swot->rowAttributes() ?>>
        <label id="elh_analise_swot_idanalise_swot" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idanalise_swot->caption() ?><?= $Page->idanalise_swot->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idanalise_swot->cellAttributes() ?>>
<span id="el_analise_swot_idanalise_swot">
<span<?= $Page->idanalise_swot->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idanalise_swot->getDisplayValue($Page->idanalise_swot->EditValue))) ?>"></span>
<input type="hidden" data-table="analise_swot" data-field="x_idanalise_swot" data-hidden="1" name="x_idanalise_swot" id="x_idanalise_swot" value="<?= HtmlEncode($Page->idanalise_swot->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fanalise_swotedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fanalise_swotedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

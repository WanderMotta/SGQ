...
$Page->showMessage();
?>
<form name="fobjetivo_qualidadeadd" id="fobjetivo_qualidadeadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_objetivo_qualidade_processo_idprocesso">
    <select
        ...
        data-select2-id="fobjetivo_qualidadeadd_x_processo_idprocesso"
        <?php } ?>
...
<?php if (!$Page->processo_idprocesso->IsNativeSelect) { ?>
<script>
loadjs.ready("fobjetivo_qualidadeadd", function() {
    var options = { name: "x_processo_idprocesso", selectId: "fobjetivo_qualidadeadd_x_processo_idprocesso" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fobjetivo_qualidadeadd.lists.processo_idprocesso?.lookupOptions.length) {
        options.data = { id: "x_processo_idprocesso", form: "fobjetivo_qualidadeadd" };
    } else {
        options.ajax = { id: "x_processo_idprocesso", form: "fobjetivo_qualidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_objetivo_qualidade_departamentos_iddepartamentos">
    <select
        ...
        data-select2-id="fobjetivo_qualidadeadd_x_departamentos_iddepartamentos"
        <?php } ?>
        data-table="objetivo_qualidade"
...
<script>
loadjs.ready("fobjetivo_qualidadeadd", function() {
    var options = { name: "x_departamentos_iddepartamentos", selectId: "fobjetivo_qualidadeadd_x_departamentos_iddepartamentos" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fobjetivo_qualidadeadd.lists.departamentos_iddepartamentos?.lookupOptions.length) {
        options.data = { id: "x_departamentos_iddepartamentos", form: "fobjetivo_qualidadeadd" };
    } else {
        options.ajax = { id: "x_departamentos_iddepartamentos", form: "fobjetivo_qualidadeadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fobjetivo_qualidadeadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fobjetivo_qualidadeadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

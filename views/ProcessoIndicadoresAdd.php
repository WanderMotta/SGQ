...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fprocesso_indicadoresadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fprocesso_indicadoresadd" id="fprocesso_indicadoresadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_processo_indicadores_processo_idprocesso">
    <select
        ...
        data-select2-id="fprocesso_indicadoresadd_x_processo_idprocesso"
        <?php } ?>
...
<?php if (!$Page->processo_idprocesso->IsNativeSelect) { ?>
<script>
loadjs.ready("fprocesso_indicadoresadd", function() {
    var options = { name: "x_processo_idprocesso", selectId: "fprocesso_indicadoresadd_x_processo_idprocesso" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprocesso_indicadoresadd.lists.processo_idprocesso?.lookupOptions.length) {
        options.data = { id: "x_processo_idprocesso", form: "fprocesso_indicadoresadd" };
    } else {
        options.ajax = { id: "x_processo_idprocesso", form: "fprocesso_indicadoresadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_processo_indicadores_indicadores_idindicadores">
    <select
        ...
        data-select2-id="fprocesso_indicadoresadd_x_indicadores_idindicadores"
        <?php } ?>
        data-table="processo_indicadores"
...
<script>
loadjs.ready("fprocesso_indicadoresadd", function() {
    var options = { name: "x_indicadores_idindicadores", selectId: "fprocesso_indicadoresadd_x_indicadores_idindicadores" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprocesso_indicadoresadd.lists.indicadores_idindicadores?.lookupOptions.length) {
        options.data = { id: "x_indicadores_idindicadores", form: "fprocesso_indicadoresadd" };
    } else {
        options.ajax = { id: "x_indicadores_idindicadores", form: "fprocesso_indicadoresadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fprocesso_indicadoresadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fprocesso_indicadoresadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

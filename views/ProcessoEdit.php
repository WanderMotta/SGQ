...
?>
<main class="edit">
<form name="fprocessoedit" id="fprocessoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["idprocesso", [fields.idprocesso.visible && fields.idprocesso.required ? ew.Validators.required(fields.idprocesso.caption) : null], fields.idprocesso.isInvalid],
...
<div class="ew-nav<?= $Page->MultiPages->containerClasses() ?>" id="pages_ProcessoEdit"><!-- multi-page tabs -->
...
<?php if ($Page->idprocesso->Visible) { // idprocesso ?>
    <div id="r_idprocesso"<?= $Page->idprocesso->rowAttributes() ?>>
        <label id="elh_processo_idprocesso" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idprocesso->caption() ?><?= $Page->idprocesso->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idprocesso->cellAttributes() ?>>
<span id="el_processo_idprocesso">
<span<?= $Page->idprocesso->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idprocesso->getDisplayValue($Page->idprocesso->EditValue))) ?>"></span>
<input type="hidden" data-table="processo" data-field="x_idprocesso" data-hidden="1" data-page="1" name="x_idprocesso" id="x_idprocesso" value="<?= HtmlEncode($Page->idprocesso->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fprocessoedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fprocessoedit", "x_dt_cadastro", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<input type="hidden" name="fa_x_proced_int_trabalho" id= "fa_x_proced_int_trabalho" value="<?= (Post("fa_x_proced_int_trabalho") == "0") ? "0" : "1" ?>">
...
<input type="hidden" name="fa_x_mapa" id= "fa_x_mapa" value="<?= (Post("fa_x_mapa") == "0") ? "0" : "1" ?>">
...
<input type="hidden" name="fa_x_formulario" id= "fa_x_formulario" value="<?= (Post("fa_x_formulario") == "0") ? "0" : "1" ?>">
...
<?php
    if (in_array("processo_indicadores", explode(",", $Page->getCurrentDetailTable())) && $processo_indicadores->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fprocessoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fprocessoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

...
// Page object
$AnaliseCriticaDirecaoEdit = &$Page;
?>
...
?>
<main class="edit">
<form name="fanalise_critica_direcaoedit" id="fanalise_critica_direcaoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { analise_critica_direcao: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fanalise_critica_direcaoedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fanalise_critica_direcaoedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idanalise_critica_direcao", [fields.idanalise_critica_direcao.visible && fields.idanalise_critica_direcao.required ? ew.Validators.required(fields.idanalise_critica_direcao.caption) : null], fields.idanalise_critica_direcao.isInvalid],
...
<div class="ew-nav<?= $Page->MultiPages->containerClasses() ?>" id="pages_AnaliseCriticaDirecaoEdit"><!-- multi-page tabs -->
...
<?php if ($Page->idanalise_critica_direcao->Visible) { // idanalise_critica_direcao ?>
    <div id="r_idanalise_critica_direcao"<?= $Page->idanalise_critica_direcao->rowAttributes() ?>>
        <label id="elh_analise_critica_direcao_idanalise_critica_direcao" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idanalise_critica_direcao->caption() ?><?= $Page->idanalise_critica_direcao->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idanalise_critica_direcao->cellAttributes() ?>>
<span id="el_analise_critica_direcao_idanalise_critica_direcao">
<span<?= $Page->idanalise_critica_direcao->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idanalise_critica_direcao->getDisplayValue($Page->idanalise_critica_direcao->EditValue))) ?>"></span>
<input type="hidden" data-table="analise_critica_direcao" data-field="x_idanalise_critica_direcao" data-hidden="1" data-page="1" name="x_idanalise_critica_direcao" id="x_idanalise_critica_direcao" value="<?= HtmlEncode($Page->idanalise_critica_direcao->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fanalise_critica_direcaoedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fanalise_critica_direcaoedit", "x_data", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_analise_critica_direcao_participantes">
    <select
        ...
        data-select2-id="fanalise_critica_direcaoedit_x_participantes[]"
...
<script>
loadjs.ready("fanalise_critica_direcaoedit", function() {
    var options = { name: "x_participantes[]", selectId: "fanalise_critica_direcaoedit_x_participantes[]" };
    options.multiple = true;
    if (fanalise_critica_direcaoedit.lists.participantes?.lookupOptions.length) {
        options.data = { id: "x_participantes[]", form: "fanalise_critica_direcaoedit" };
    } else {
        options.ajax = { id: "x_participantes[]", form: "fanalise_critica_direcaoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_analise_critica_direcao_usuario_idusuario">
    <select
        ...
        data-select2-id="fanalise_critica_direcaoedit_x_usuario_idusuario"
        <?php } ?>
        data-table="analise_critica_direcao"
...
<script>
loadjs.ready("fanalise_critica_direcaoedit", function() {
    var options = { name: "x_usuario_idusuario", selectId: "fanalise_critica_direcaoedit_x_usuario_idusuario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fanalise_critica_direcaoedit.lists.usuario_idusuario?.lookupOptions.length) {
        options.data = { id: "x_usuario_idusuario", form: "fanalise_critica_direcaoedit" };
    } else {
        options.ajax = { id: "x_usuario_idusuario", form: "fanalise_critica_direcaoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fanalise_critica_direcaoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fanalise_critica_direcaoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

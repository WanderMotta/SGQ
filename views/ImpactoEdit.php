...
?>
<main class="edit">
<form name="fimpactoedit" id="fimpactoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["idimpacto", [fields.idimpacto.visible && fields.idimpacto.required ? ew.Validators.required(fields.idimpacto.caption) : null], fields.idimpacto.isInvalid],
...
<?php if ($Page->idimpacto->Visible) { // idimpacto ?>
    <div id="r_idimpacto"<?= $Page->idimpacto->rowAttributes() ?>>
        <label id="elh_impacto_idimpacto" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idimpacto->caption() ?><?= $Page->idimpacto->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idimpacto->cellAttributes() ?>>
<span id="el_impacto_idimpacto">
<span<?= $Page->idimpacto->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idimpacto->getDisplayValue($Page->idimpacto->EditValue))) ?>"></span>
<input type="hidden" data-table="impacto" data-field="x_idimpacto" data-hidden="1" name="x_idimpacto" id="x_idimpacto" value="<?= HtmlEncode($Page->idimpacto->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fimpactoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fimpactoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->

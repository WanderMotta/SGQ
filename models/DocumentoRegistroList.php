 ...
 * Page class
 */
class DocumentoRegistroList extends DocumentoRegistro
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentoRegistroList";
    ...
    public $FormName = "fdocumento_registrolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "DocumentoRegistroList";
    ...
    {
        $this->iddocumento_registro->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "DocumentoRegistroAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "DocumentoRegistroDelete";
        $this->MultiUpdateUrl = "DocumentoRegistroUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fdocumento_registrogrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->iddocumento_registro->AdvancedSearch->toJson(), ","); // Field iddocumento_registro
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fdocumento_registrosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field iddocumento_registro
        $this->iddocumento_registro->AdvancedSearch->SearchValue = @$filter["x_iddocumento_registro"];
        $this->iddocumento_registro->AdvancedSearch->SearchOperator = @$filter["z_iddocumento_registro"];
        $this->iddocumento_registro->AdvancedSearch->SearchCondition = @$filter["v_iddocumento_registro"];
        $this->iddocumento_registro->AdvancedSearch->SearchValue2 = @$filter["y_iddocumento_registro"];
        $this->iddocumento_registro->AdvancedSearch->SearchOperator2 = @$filter["w_iddocumento_registro"];
        $this->iddocumento_registro->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->iddocumento_registro, $default, false); // iddocumento_registro
        ...
        if (!$default && $this->Command == "search") {
            $this->iddocumento_registro->AdvancedSearch->save(); // iddocumento_registro
    ...
    public function showFilterList()
    {
        ...
        // Field iddocumento_registro
        $filter = $this->queryBuilderWhere("iddocumento_registro");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->iddocumento_registro, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->iddocumento_registro->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->iddocumento_registro->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->iddocumento_registro->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->iddocumento_registro->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->iddocumento_registro->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->iddocumento_registro, $ctrl); // iddocumento_registro
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->iddocumento_registro->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"documento_registro\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"documento_registro\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"documento_registro\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdocumento_registrolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdocumento_registrolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->iddocumento_registro->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"documento_registro\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "iddocumento_registro");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdocumento_registrosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdocumento_registrosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdocumento_registrolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_documento_registro", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_documento_registro",
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->iddocumento_registro->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->iddocumento_registro->AdvancedSearch->SearchValue != "" || $this->iddocumento_registro->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->isExport()) {
                $this->iddocumento_registro->ViewValue = $this->highlightValue($this->iddocumento_registro);
            }
            ...
            $this->iddocumento_registro->EditValue = $this->iddocumento_registro->AdvancedSearch->SearchValue;
            $this->iddocumento_registro->PlaceHolder = RemoveHtml($this->iddocumento_registro->caption());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdocumento_registrosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-table=\"documento_registro\" data-caption=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-ew-action=\"modal\" data-url=\"DocumentoRegistroSearch\" data-btn=\"SearchBtn\">" . $Language->phrase("AdvancedSearch", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-caption=\"" . $Language->phrase("AdvancedSearch", true) . "\" href=\"DocumentoRegistroSearch\">" . $Language->phrase("AdvancedSearch", false) . "</a>";
        }
        $item->Visible = true;

        // Search highlight button
        $item = &$this->SearchOptions->add("searchhighlight");
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"fdocumento_registrosrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";

 ...
 * Page class
 */
class ItemPlanoAudIntView extends ItemPlanoAudInt
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ItemPlanoAudIntView";
    ...
    // CSS class/style
    public $CurrentPageName = "ItemPlanoAudIntView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("iditem_plano_aud_int") ?? Route("iditem_plano_aud_int")) !== null) {
            $this->RecKey["iditem_plano_aud_int"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("iditem_plano_aud_int") ?? Route("iditem_plano_aud_int")) !== null) {
            ...
            $this->RecKey["iditem_plano_aud_int"] = $this->iditem_plano_aud_int->QueryStringValue;
        } elseif (Post("iditem_plano_aud_int") !== null) {
            $this->iditem_plano_aud_int->setFormValue(Post("iditem_plano_aud_int"));
            $this->RecKey["iditem_plano_aud_int"] = $this->iditem_plano_aud_int->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->iditem_plano_aud_int->setQueryStringValue($keyValue);
            $this->RecKey["iditem_plano_aud_int"] = $this->iditem_plano_aud_int->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ItemPlanoAudIntList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ItemPlanoAudIntList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->iditem_plano_aud_int->CurrentValue, $this->CurrentRow['iditem_plano_aud_int'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ItemPlanoAudIntList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_relatorio_aud_int");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("relatorio_aud_int", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("RelatorioAudIntList?" . Config("TABLE_SHOW_MASTER") . "=item_plano_aud_int&" . GetForeignKeyUrl("fk_iditem_plano_aud_int", $this->iditem_plano_aud_int->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("RelatorioAudIntGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'item_plano_aud_int')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=relatorio_aud_int"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "relatorio_aud_int";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'item_plano_aud_int')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=relatorio_aud_int"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "relatorio_aud_int";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'item_plano_aud_int')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=relatorio_aud_int"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (relatorio_aud_int)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("relatorio_aud_int", explode(",", $this->getCurrentDetailTable()))) {
            $relatorio_aud_int = new RelatorioAudIntList();
            $rsdetail = $relatorio_aud_int->loadRs($relatorio_aud_int->getDetailFilterFromSession(), $relatorio_aud_int->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($relatorio_aud_int);
                    $relatorio_aud_int->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }

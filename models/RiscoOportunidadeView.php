 ...
 * Page class
 */
class RiscoOportunidadeView extends RiscoOportunidade
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RiscoOportunidadeView";
    ...
    // CSS class/style
    public $CurrentPageName = "RiscoOportunidadeView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("idrisco_oportunidade") ?? Route("idrisco_oportunidade")) !== null) {
            $this->RecKey["idrisco_oportunidade"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("idrisco_oportunidade") ?? Route("idrisco_oportunidade")) !== null) {
            ...
            $this->RecKey["idrisco_oportunidade"] = $this->idrisco_oportunidade->QueryStringValue;
        } elseif (Post("idrisco_oportunidade") !== null) {
            $this->idrisco_oportunidade->setFormValue(Post("idrisco_oportunidade"));
            $this->RecKey["idrisco_oportunidade"] = $this->idrisco_oportunidade->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->idrisco_oportunidade->setQueryStringValue($keyValue);
            $this->RecKey["idrisco_oportunidade"] = $this->idrisco_oportunidade->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "RiscoOportunidadeList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("RiscoOportunidadeList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->idrisco_oportunidade->CurrentValue, $this->CurrentRow['idrisco_oportunidade'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "RiscoOportunidadeList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_plano_acao"
        $item = &$option->add("detail_plano_acao");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("plano_acao", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("PlanoAcaoList?" . Config("TABLE_SHOW_MASTER") . "=risco_oportunidade&" . GetForeignKeyUrl("fk_idrisco_oportunidade", $this->idrisco_oportunidade->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("PlanoAcaoGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'risco_oportunidade')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=plano_acao"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "plano_acao";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'risco_oportunidade')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=plano_acao"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "plano_acao";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'risco_oportunidade')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=plano_acao"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (plano_acao)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("plano_acao", explode(",", $this->getCurrentDetailTable()))) {
            $plano_acao = new PlanoAcaoList();
            $rsdetail = $plano_acao->loadRs($plano_acao->getDetailFilterFromSession(), $plano_acao->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($plano_acao);
                    $plano_acao->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }

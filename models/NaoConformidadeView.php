 ...
 * Page class
 */
class NaoConformidadeView extends NaoConformidade
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NaoConformidadeView";
    ...
    // CSS class/style
    public $CurrentPageName = "NaoConformidadeView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("idnao_conformidade") ?? Route("idnao_conformidade")) !== null) {
            $this->RecKey["idnao_conformidade"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("idnao_conformidade") ?? Route("idnao_conformidade")) !== null) {
            ...
            $this->RecKey["idnao_conformidade"] = $this->idnao_conformidade->QueryStringValue;
        } elseif (Post("idnao_conformidade") !== null) {
            $this->idnao_conformidade->setFormValue(Post("idnao_conformidade"));
            $this->RecKey["idnao_conformidade"] = $this->idnao_conformidade->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->idnao_conformidade->setQueryStringValue($keyValue);
            $this->RecKey["idnao_conformidade"] = $this->idnao_conformidade->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "NaoConformidadeList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("NaoConformidadeList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->idnao_conformidade->CurrentValue, $this->CurrentRow['idnao_conformidade'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "NaoConformidadeList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_plano_acao_nc");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("plano_acao_nc", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("PlanoAcaoNcList?" . Config("TABLE_SHOW_MASTER") . "=nao_conformidade&" . GetForeignKeyUrl("fk_idnao_conformidade", $this->idnao_conformidade->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("PlanoAcaoNcGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'nao_conformidade')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=plano_acao_nc"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "plano_acao_nc";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'nao_conformidade')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=plano_acao_nc"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "plano_acao_nc";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'nao_conformidade')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=plano_acao_nc"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (plano_acao_nc)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("plano_acao_nc", explode(",", $this->getCurrentDetailTable()))) {
            $plano_acao_nc = new PlanoAcaoNcList();
            $rsdetail = $plano_acao_nc->loadRs($plano_acao_nc->getDetailFilterFromSession(), $plano_acao_nc->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($plano_acao_nc);
                    $plano_acao_nc->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }

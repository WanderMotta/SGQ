...
class Dashboard2 extends ReportTable
{
    ...
    public $ItemClassNames = ["col-sm-auto","col-sm-auto","col-sm-auto","col-sm-auto","col-sm-auto"];
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = Container("GraficosList");
                $Page->run();
                $Page->EvidenciasxMes->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->EvidenciasxMes->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 2) {
                $Page = Container("IndicadoresList");
                $Page->run();
                $Page->EvidenciaxMensal->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->EvidenciaxMensal->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 3) {
                $Page = Container("RelCrossGraficoCrosstab");
                $Page->run();
                $Page->EvidenciaxMensal->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->EvidenciaxMensal->render("ew-dashboard-chart", null, null) .
            ...
            if ($id == 4) {
                $Page = Container("RiscoOportunidadeList");
                $Page->run();
                $Page->Severidade->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->Severidade->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 5) {
                $Page = Container("RiscoOportunidadeList");
                $Page->run();
                $Page->GrauAtencao->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->GrauAtencao->render("ew-dashboard-chart", null, null) .
                    GetDebugMessage(); // Return chart content
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;

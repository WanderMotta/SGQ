 ...
 * Page class
 */
class RelCrossGraficoCrosstab extends RelCrossGrafico
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RelCrossGraficoCrosstab";
    ...
    public $CurrentPageName = "RelCrossGrafico";
    ...
    public function __construct()
    {
        ...
        // Table object (rel_cross_grafico)
        if (!isset($GLOBALS["rel_cross_grafico"]) || $GLOBALS["rel_cross_grafico"]::class == PROJECT_NAMESPACE . "rel_cross_grafico") {
            $GLOBALS["rel_cross_grafico"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'rel_cross_grafico');
        }
    ...
    public function run()
    {
        ...
        if ($year != "") {
            AddFilter($this->SearchWhere, "YEAR(`data_base`) = " . QuotedValue($year, DataType::STRING, $this->Dbid));
        }
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            if ($this->indicadores_idindicadores->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->indicadores_idindicadores->Lookup->getTable()->Fields["idindicadores"]->searchExpression(), "=", $this->indicadores_idindicadores->AdvancedSearch->SearchValue, $this->indicadores_idindicadores->Lookup->getTable()->Fields["idindicadores"]->searchDataType(), "");
                }
            ...
            $this->indicadores_idindicadores->CellCssStyle .= "text-align: left;";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frel_cross_graficosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frel_cross_graficosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frel_cross_graficosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function getFilterList()
    {
        ...
        // Field indicadores_idindicadores
        $wrk = "";
        $wrk = $this->indicadores_idindicadores->AdvancedSearch->SearchValue;
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {

 ...
 * Page class
 */
class AnaliseSwotGrid extends AnaliseSwot
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AnaliseSwotGrid";
    ...
    public $FormName = "fanalise_swotgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "AnaliseSwotGrid";
    ...
    public function setVisibility()
    {
        ...
        $this->contexto_idcontexto->Visible = false;
    }
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fanalise_swotgrid";
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "contexto") {
            $masterTbl = Container("contexto");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ContextoList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idanalise_swot->CurrentValue;
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_fatores") &&
            $CurrentForm->hasValue("o_fatores") &&
            $this->fatores->CurrentValue != $this->fatores->DefaultValue &&
            !($this->fatores->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->fatores->CurrentValue == $this->fatores->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_ponto") &&
            $CurrentForm->hasValue("o_ponto") &&
            $this->ponto->CurrentValue != $this->ponto->DefaultValue &&
            !($this->ponto->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->ponto->CurrentValue == $this->ponto->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_analise") &&
            $CurrentForm->hasValue("o_analise") &&
            $this->analise->CurrentValue != $this->analise->DefaultValue &&
            !($this->analise->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->analise->CurrentValue == $this->analise->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_impacto_idimpacto") &&
            $CurrentForm->hasValue("o_impacto_idimpacto") &&
            $this->impacto_idimpacto->CurrentValue != $this->impacto_idimpacto->DefaultValue &&
            !($this->impacto_idimpacto->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->impacto_idimpacto->CurrentValue == $this->impacto_idimpacto->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->fatores->Expression . " ASC" . ", " . $this->ponto->Expression . " ASC" . ", " . $this->impacto_idimpacto->Expression . " ASC"; // Set up default sort
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"analise_swot\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"analise_swot\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"analise_swot\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->CurrentMode == "view") { // Check view mode
            ...
            if ($this->ModalAdd && !IsMobile()) {
                $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"analise_swot\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
            } else {
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_analise_swot", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_analise_swot",
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_fatores")) {
            $this->fatores->setOldValue($CurrentForm->getValue("o_fatores"));
        }
        ...
        if ($CurrentForm->hasValue("o_ponto")) {
            $this->ponto->setOldValue($CurrentForm->getValue("o_ponto"));
        }
        ...
        if ($CurrentForm->hasValue("o_analise")) {
            $this->analise->setOldValue($CurrentForm->getValue("o_analise"));
        }
        ...
        if ($CurrentForm->hasValue("o_impacto_idimpacto")) {
            $this->impacto_idimpacto->setOldValue($CurrentForm->getValue("o_impacto_idimpacto"));
        }
        ...
        if (!$this->idanalise_swot->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->idanalise_swot->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->idanalise_swot->CurrentValue = $this->idanalise_swot->FormValue;
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->isExport()) {
                $this->analise->ViewValue = $this->highlightValue($this->analise);
            }
            ...
            $this->impacto_idimpacto->HrefValue = "";
                ...
                $this->analise->Count = 0; // Initialize count
    ...
    protected function editRow()
    {
        ...
        // Check referential integrity for master table 'contexto'
        $detailKeys = [];
        $keyValue = $rsnew['contexto_idcontexto'] ?? $rsold['contexto_idcontexto'];
        $detailKeys['contexto_idcontexto'] = $keyValue;
    ...
    protected function getEditRow($rsold)
    {
        ...
        // fatores
        $this->fatores->setDbValueDef($rsnew, $this->fatores->CurrentValue, $this->fatores->ReadOnly);

        // ponto
        $this->ponto->setDbValueDef($rsnew, $this->ponto->CurrentValue, $this->ponto->ReadOnly);

        // analise
        $this->analise->setDbValueDef($rsnew, $this->analise->CurrentValue, $this->analise->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['fatores'])) { // fatores
            $this->fatores->CurrentValue = $row['fatores'];
        }
        if (isset($row['ponto'])) { // ponto
            $this->ponto->CurrentValue = $row['ponto'];
        }
        if (isset($row['analise'])) { // analise
            $this->analise->CurrentValue = $row['analise'];
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "contexto") {
            $this->contexto_idcontexto->Visible = true; // Need to insert foreign key
            $this->contexto_idcontexto->CurrentValue = $this->contexto_idcontexto->getSessionValue();
        }
        ...
        $detailKeys = [];
        $detailKeys["contexto_idcontexto"] = $this->contexto_idcontexto->getSessionValue();
        $masterTable = Container("contexto");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
    ...
    protected function getAddRow()
    {
        ...
        if ($this->contexto_idcontexto->getSessionValue() != "") {
            $rsnew['contexto_idcontexto'] = $this->contexto_idcontexto->getSessionValue();
        }
        return $rsnew;
    }

...
class UsuarioAdd extends Usuario
{
    ...
    // CSS class/style
    public $CurrentPageName = "UsuarioAdd";
    ...
    protected function addRow($rsold = null)
    {
        ...
        if (
            ...
            !$Security->isValidUserID($this->idusuario->CurrentValue)
        ) {
            $userIdMsg = str_replace("%c", CurrentUserID(), $Language->phrase("UnAuthorizedUserID"));
            $userIdMsg = str_replace("%u", strval($this->idusuario->CurrentValue), $userIdMsg);
            $this->setFailureMessage($userIdMsg);
            return false;
        }
        if ($this->_login->CurrentValue != "") { // Check field with unique index
            $filter = "(`login` = '" . AdjustSql($this->_login->CurrentValue, $this->Dbid) . "')";
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['nome'])) { // nome
            $this->nome->setFormValue($row['nome']);
        }
        if (isset($row['login'])) { // login
            $this->_login->setFormValue($row['login']);
        }
        if (isset($row['senha'])) { // senha
            $this->senha->setFormValue($row['senha']);
        }
        if (isset($row['status'])) { // status
            $this->status->setFormValue($row['status']);
        }
        if (isset($row['idusuario'])) { // idusuario
            $this->idusuario->setFormValue($row['idusuario']);
        }
    }

...
class UsuarioEdit extends Usuario
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idusuario") ?? Key(0) ?? Route(2)) !== null) {
                $this->idusuario->setQueryStringValue($keyValue);
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idusuario") ?? Route("idusuario")) !== null) {
                    $this->idusuario->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
    ...
    public function renderRow()
    {
        ...
        // idusuario
        $this->idusuario->RowCssClass = "row";

        // nome
        $this->nome->RowCssClass = "row";

        // login
        $this->_login->RowCssClass = "row";

        // senha
        $this->senha->RowCssClass = "row";
            ...
            if (!$Security->canAdmin()) { // System admin
                ...
                if ($this->status->ViewValue !== null) { // Load from cache
                    ...
                    if ($curVal == "") {
                        ...
                        $filterWrk = SearchFilter($this->status->Lookup->getTable()->Fields["UserLevelID"]->searchExpression(), "=", $this->status->CurrentValue, $this->status->Lookup->getTable()->Fields["UserLevelID"]->searchDataType(), "");
                    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idusuario->Visible && $this->idusuario->Required) {
                if (!$this->idusuario->IsDetailKey && EmptyValue($this->idusuario->FormValue)) {
                    $this->idusuario->addErrorMessage(str_replace("%s", $this->idusuario->caption(), $this->idusuario->RequiredErrorMessage));
                }
            }
            if ($this->nome->Visible && $this->nome->Required) {
            ...
            if ($this->_login->Visible && $this->_login->Required) {
                if (!$this->_login->IsDetailKey && EmptyValue($this->_login->FormValue)) {
                    $this->_login->addErrorMessage(str_replace("%s", $this->_login->caption(), $this->_login->RequiredErrorMessage));
                }
            }
            if (!$this->_login->Raw && Config("REMOVE_XSS") && CheckUsername($this->_login->FormValue)) {
                $this->_login->addErrorMessage($Language->phrase("InvalidUsernameChars"));
            }
            if ($this->senha->Visible && $this->senha->Required) {
                if (!$this->senha->IsDetailKey && EmptyValue($this->senha->FormValue)) {
                    $this->senha->addErrorMessage(str_replace("%s", $this->senha->caption(), $this->senha->RequiredErrorMessage));
                }
            }
            if (!$this->senha->Raw && Config("REMOVE_XSS") && CheckPassword($this->senha->FormValue)) {
                $this->senha->addErrorMessage($Language->phrase("InvalidPasswordChars"));
            }
            if ($this->status->Visible && $this->status->Required) {
                if ($Security->canAdmin() && !$this->status->IsDetailKey && EmptyValue($this->status->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (login)
        if ($this->_login->CurrentValue != "") {
            $filterChk = "(`login` = '" . AdjustSql($this->_login->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->_login->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->_login->CurrentValue, $idxErrMsg);
    ...
    protected function getEditRow($rsold)
    {
        ...
        // login
        $this->_login->setDbValueDef($rsnew, $this->_login->CurrentValue, $this->_login->ReadOnly);

        // senha
        if (!IsMaskedPassword($this->senha->CurrentValue)) {
            $this->senha->setDbValueDef($rsnew, $this->senha->CurrentValue, $this->senha->ReadOnly || Config("ENCRYPTED_PASSWORD") && $rsold['senha'] == $this->senha->CurrentValue);
        }
    ...
    {
        if (isset($row['nome'])) { // nome
            $this->nome->CurrentValue = $row['nome'];
        }
        if (isset($row['login'])) { // login
            $this->_login->CurrentValue = $row['login'];
        }
        if (isset($row['senha'])) { // senha
            $this->senha->CurrentValue = $row['senha'];
        }

...
class Usuario extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // idusuario
        $this->idusuario = new DbField(
            $this, // Table
            'x_idusuario', // Variable name
            'idusuario', // Name
            '`idusuario`', // Expression
            '`idusuario`', // Basic search expression
            19, // Type
            2, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`idusuario`', // Virtual expression
            false, // Is virtual
        ...
        $this->idusuario->InputTextType = "text";
        $this->idusuario->Raw = true;
        $this->idusuario->IsAutoIncrement = true; // Autoincrement field
        $this->idusuario->IsPrimaryKey = true; // Primary key field
        $this->idusuario->Nullable = false; // NOT NULL field
        $this->idusuario->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->idusuario->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['idusuario'] = &$this->idusuario;

        // nome
        $this->nome = new DbField(
            $this, // Table
            'x_nome', // Variable name
            'nome', // Name
            '`nome`', // Expression
            '`nome`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nome`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nome->InputTextType = "text";
        $this->nome->Nullable = false; // NOT NULL field
        $this->nome->Required = true; // Required field
        $this->nome->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['nome'] = &$this->nome;

        // login
        $this->_login = new DbField(
            $this, // Table
            'x__login', // Variable name
            'login', // Name
            '`login`', // Expression
            '`login`', // Basic search expression
            200, // Type
            25, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`login`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->_login->Raw = true;
        $this->_login->Nullable = false; // NOT NULL field
        $this->_login->Required = true; // Required field
        $this->_login->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['login'] = &$this->_login;

        // senha
        $this->senha = new DbField(
            $this, // Table
            'x_senha', // Variable name
            'senha', // Name
            '`senha`', // Expression
            '`senha`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`senha`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->status->Lookup = new Lookup($this->status, 'UserLevels', false, 'UserLevelID', ["UserLevelName","","",""], '', '', [], [], [], [], [], [], false, '', '', "`UserLevelName`");
        ...
        $this->ativo->Lookup = new Lookup($this->ativo, 'usuario', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->idusuario->setDbValue($conn->lastInsertId());
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['idusuario']) && !EmptyValue($this->idusuario->CurrentValue)) {
                $rs['idusuario'] = $this->idusuario->CurrentValue;
            }
        }
        return $success;
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->idusuario->CurrentValue : $this->idusuario->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->idusuario->CurrentValue = $keys[0];
            } else {
                $this->idusuario->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->idusuario->OldValue) && !$current ? $this->idusuario->OldValue : $this->idusuario->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "UsuarioView" => $Language->phrase("View"),
            "UsuarioEdit" => $Language->phrase("Edit"),
            "UsuarioAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "UsuarioView",
            Config("API_ADD_ACTION") => "UsuarioAdd",
            Config("API_EDIT_ACTION") => "UsuarioEdit",
            Config("API_DELETE_ACTION") => "UsuarioDelete",
            Config("API_LIST_ACTION") => "UsuarioList",
            default => ""
        };
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("UsuarioDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"idusuario\":" . VarToJson($this->idusuario->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->idusuario->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->idusuario->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("idusuario") ?? Route("idusuario")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function renderEditRow()
    {
        ...
        // idusuario
        $this->idusuario->setupEditAttributes();
        $this->idusuario->EditValue = $this->idusuario->CurrentValue;

        // nome
        $this->nome->setupEditAttributes();
        if (!$this->nome->Raw) {
        ...
        // senha
        $this->senha->setupEditAttributes();
        $this->senha->EditValue = $Language->phrase("PasswordMask"); // Show as masked password
        $this->senha->PlaceHolder = RemoveHtml($this->senha->caption());

        // status
        $this->status->setupEditAttributes();
        if (!$Security->canAdmin()) { // System admin
            $this->status->EditValue = $Language->phrase("PasswordMask");
        } else {
    ...
    {
        global $Security;
        $userIdFilter = '`idusuario` = ' . QuotedValue($userId, DataType::NUMBER, Config("USER_TABLE_DBID"));
        return $userIdFilter;
    }
    ...
    public function addUserIDFilter($filter = "", $id = "")
    {
        ...
        if (!$this->userIDAllow($id) && !$Security->isAdmin()) {
            ...
            if ($filterWrk != "") {
                $filterWrk = '`idusuario` IN (' . $filterWrk . ')';
            }
        }
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM usuario";

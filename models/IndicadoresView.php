 ...
 * Page class
 */
class IndicadoresView extends Indicadores
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IndicadoresView";
    ...
    // CSS class/style
    public $CurrentPageName = "IndicadoresView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("idindicadores") ?? Route("idindicadores")) !== null) {
            $this->RecKey["idindicadores"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("idindicadores") ?? Route("idindicadores")) !== null) {
            ...
            $this->RecKey["idindicadores"] = $this->idindicadores->QueryStringValue;
        } elseif (Post("idindicadores") !== null) {
            $this->idindicadores->setFormValue(Post("idindicadores"));
            $this->RecKey["idindicadores"] = $this->idindicadores->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->idindicadores->setQueryStringValue($keyValue);
            $this->RecKey["idindicadores"] = $this->idindicadores->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "IndicadoresList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("IndicadoresList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->idindicadores->CurrentValue, $this->CurrentRow['idindicadores'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "IndicadoresList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_graficos"
        $item = &$option->add("detail_graficos");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("graficos", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("GraficosList?" . Config("TABLE_SHOW_MASTER") . "=indicadores&" . GetForeignKeyUrl("fk_idindicadores", $this->idindicadores->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("GraficosGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'indicadores')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=graficos"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "graficos";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'indicadores')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=graficos"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "graficos";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'indicadores')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=graficos"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (graficos)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("graficos", explode(",", $this->getCurrentDetailTable()))) {
            $graficos = new GraficosList();
            $rsdetail = $graficos->loadRs($graficos->getDetailFilterFromSession(), $graficos->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($graficos);
                    $graficos->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }

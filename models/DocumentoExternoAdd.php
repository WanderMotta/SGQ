 ...
 * Page class
 */
class DocumentoExternoAdd extends DocumentoExterno
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentoExternoAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentoExternoAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "DocumentoExternoView") {
    ...
    {
        $this->distribuicao->DefaultValue = $this->distribuicao->getDefault(); // PHP
        $this->distribuicao->OldValue = $this->distribuicao->DefaultValue;
        $this->tem_validade->DefaultValue = $this->tem_validade->getDefault(); // PHP
        $this->tem_validade->OldValue = $this->tem_validade->DefaultValue;
    ...
    protected function getAddRow()
    {
        ...
        // distribuicao
        $this->distribuicao->setDbValueDef($rsnew, $this->distribuicao->CurrentValue, strval($this->distribuicao->CurrentValue) == "");

        // tem_validade
        $this->tem_validade->setDbValueDef($rsnew, $this->tem_validade->CurrentValue, strval($this->tem_validade->CurrentValue) == "");

        // valido_ate
        $this->valido_ate->setDbValueDef($rsnew, UnFormatDateTime($this->valido_ate->CurrentValue, $this->valido_ate->formatPattern()), false);
        ...
        // localizacao_idlocalizacao
        $this->localizacao_idlocalizacao->setDbValueDef($rsnew, $this->localizacao_idlocalizacao->CurrentValue, false);

        // usuario_responsavel
        $this->usuario_responsavel->setDbValueDef($rsnew, $this->usuario_responsavel->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['distribuicao'])) { // distribuicao
            $this->distribuicao->setFormValue($row['distribuicao']);
        }
        if (isset($row['tem_validade'])) { // tem_validade
            $this->tem_validade->setFormValue($row['tem_validade']);
        }
        if (isset($row['valido_ate'])) { // valido_ate
            $this->valido_ate->setFormValue($row['valido_ate']);
        }
        ...
        if (isset($row['localizacao_idlocalizacao'])) { // localizacao_idlocalizacao
            $this->localizacao_idlocalizacao->setFormValue($row['localizacao_idlocalizacao']);
        }
        if (isset($row['usuario_responsavel'])) { // usuario_responsavel
            $this->usuario_responsavel->setFormValue($row['usuario_responsavel']);
        }

...
/**
 * Table class for indicadores
 */
class Indicadores extends DbTable
{
    protected $SqlFrom = "";
    ...
    public $unidade_medida_idunidade_medida;
    public $meta;
    ...
    public function __construct()
    {
        ...
        // idindicadores
        $this->idindicadores = new DbField(
            $this, // Table
            'x_idindicadores', // Variable name
            'idindicadores', // Name
            '`idindicadores`', // Expression
            '`idindicadores`', // Basic search expression
            19, // Type
            2, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`idindicadores`', // Virtual expression
            false, // Is virtual
        ...
        $this->idindicadores->InputTextType = "text";
        $this->idindicadores->Raw = true;
        $this->idindicadores->IsAutoIncrement = true; // Autoincrement field
        $this->idindicadores->IsPrimaryKey = true; // Primary key field
        $this->idindicadores->IsForeignKey = true; // Foreign key field
        $this->idindicadores->Nullable = false; // NOT NULL field
        $this->idindicadores->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->idindicadores->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['idindicadores'] = &$this->idindicadores;
        ...
        $this->indicador->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        $this->unidade_medida_idunidade_medida = new DbField(
            $this, // Table
            'x_unidade_medida_idunidade_medida', // Variable name
            'unidade_medida_idunidade_medida', // Name
            '`unidade_medida_idunidade_medida`', // Expression
            '`unidade_medida_idunidade_medida`', // Basic search expression
            19, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`unidade_medida_idunidade_medida`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->unidade_medida_idunidade_medida->InputTextType = "text";
        $this->unidade_medida_idunidade_medida->Raw = true;
        $this->unidade_medida_idunidade_medida->Nullable = false; // NOT NULL field
        $this->unidade_medida_idunidade_medida->Required = true; // Required field
        $this->unidade_medida_idunidade_medida->Lookup = new Lookup($this->unidade_medida_idunidade_medida, 'unidade_medida', false, 'idunidade_medida', ["unidade","","",""], '', '', [], [], [], [], [], [], false, '', '', "`unidade`");
        $this->unidade_medida_idunidade_medida->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->unidade_medida_idunidade_medida->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['unidade_medida_idunidade_medida'] = &$this->unidade_medida_idunidade_medida;

        // meta
        $this->meta = new DbField(
            $this, // Table
            'x_meta', // Variable name
            'meta', // Name
            '`meta`', // Expression
            '`meta`', // Basic search expression
            131, // Type
            12, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`meta`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->meta->addMethod("getDefault", fn() => 0.00);
        $this->meta->InputTextType = "text";
        $this->meta->Raw = true;
        $this->meta->Nullable = false; // NOT NULL field
        $this->meta->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->meta->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['meta'] = &$this->meta;

        // Evidencia x Mensal
        $this->EvidenciaxMensal = new DbChart($this, 'EvidenciaxMensal', 'Evidencia x Mensal', 'dt_cadastro', 'indicador', 1001, '', 0, 'SUM', 600, 500);
        $this->EvidenciaxMensal->Position = 4;
        $this->EvidenciaxMensal->PageBreakType = "before";
        $this->EvidenciaxMensal->YAxisFormat = [""];
        $this->EvidenciaxMensal->YFieldFormat = [""];
        $this->EvidenciaxMensal->SortType = 0;
        $this->EvidenciaxMensal->SortSequence = "";
        $this->EvidenciaxMensal->SqlSelect = $this->getQueryBuilder()->select("`dt_cadastro`", "''", "SUM(`indicador`)");
        $this->EvidenciaxMensal->SqlGroupBy = "`dt_cadastro`";
        $this->EvidenciaxMensal->SqlOrderBy = "";
        $this->EvidenciaxMensal->SeriesDateType = "";
        $this->EvidenciaxMensal->XAxisDateFormat = 0;
        $this->EvidenciaxMensal->ID = "indicadores_EvidenciaxMensal"; // Chart ID
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "graficos") {
            $detailUrl = Container("graficos")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_idindicadores", $this->idindicadores->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "IndicadoresList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "indicadores";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->idindicadores->setDbValue($conn->lastInsertId());
            $rs['idindicadores'] = $this->idindicadores->DbValue;
        }
        return $result;
    }
    ...
    {
        // Cascade Update detail table 'graficos'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['idindicadores']) && $rsold['idindicadores'] != $rs['idindicadores'])) { // Update detail field 'indicadores_idindicadores'
            $cascadeUpdate = true;
            $rscascade['indicadores_idindicadores'] = $rs['idindicadores'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("graficos")->loadRs("`indicadores_idindicadores` = " . QuotedValue($rsold['idindicadores'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("graficos")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("graficos")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("graficos")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
        ...
        if ($success) {
            if (!isset($rs['idindicadores']) && !EmptyValue($this->idindicadores->CurrentValue)) {
                $rs['idindicadores'] = $this->idindicadores->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('idindicadores', $rs)) {
                AddFilter($where, QuotedName('idindicadores', $this->Dbid) . '=' . QuotedValue($rs['idindicadores'], $this->idindicadores->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'graficos'
        $dtlrows = Container("graficos")->loadRs("`indicadores_idindicadores` = " . QuotedValue($rs['idindicadores'], DataType::NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("graficos")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("graficos")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("graficos")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
            try {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->idindicadores->DbValue = $row['idindicadores'];
        ...
        $this->unidade_medida_idunidade_medida->DbValue = $row['unidade_medida_idunidade_medida'];
        $this->meta->DbValue = $row['meta'];
    }
    ...
    {
        return "`idindicadores` = @idindicadores@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->idindicadores->CurrentValue : $this->idindicadores->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->idindicadores->CurrentValue = $keys[0];
            } else {
                $this->idindicadores->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('idindicadores', $row) ? $row['idindicadores'] : null;
        } else {
            $val = !EmptyValue($this->idindicadores->OldValue) && !$current ? $this->idindicadores->OldValue : $this->idindicadores->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@idindicadores@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("IndicadoresList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "IndicadoresView" => $Language->phrase("View"),
            "IndicadoresEdit" => $Language->phrase("Edit"),
            "IndicadoresAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "IndicadoresView",
            Config("API_ADD_ACTION") => "IndicadoresAdd",
            Config("API_EDIT_ACTION") => "IndicadoresEdit",
            Config("API_DELETE_ACTION") => "IndicadoresDelete",
            Config("API_LIST_ACTION") => "IndicadoresList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("IndicadoresView", $parm);
        } else {
            $url = $this->keyUrl("IndicadoresView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("IndicadoresEdit", $parm);
        } else {
            $url = $this->keyUrl("IndicadoresEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("IndicadoresList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("IndicadoresAdd", $parm);
        } else {
            $url = $this->keyUrl("IndicadoresAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("IndicadoresList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("IndicadoresDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"idindicadores\":" . VarToJson($this->idindicadores->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->idindicadores->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->idindicadores->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("idindicadores") ?? Route("idindicadores")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->idindicadores->CurrentValue = $key;
            } else {
                $this->idindicadores->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->indicador->EditValue = $this->indicador->CurrentValue;
    ...
    public function exportDocument($doc, $result, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->idindicadores, '');
                $doc->exportAggregate($this->dt_cadastro, '');
                $doc->exportAggregate($this->indicador, 'COUNT');
                $doc->exportAggregate($this->periodicidade_idperiodicidade, '');
                $doc->exportAggregate($this->unidade_medida_idunidade_medida, '');
                $doc->exportAggregate($this->meta, '');
                $doc->endExportRow();
            }
        }
        if (!$doc->ExportCustom) {

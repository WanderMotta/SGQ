 ...
 * Page class
 */
class ContextoEdit extends Contexto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContextoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ContextoEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (contexto)
        if (!isset($GLOBALS["contexto"]) || $GLOBALS["contexto"]::class == PROJECT_NAMESPACE . "contexto") {
            $GLOBALS["contexto"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'contexto');
        }
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (
                    ...
                    if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                        ...
                        $result["view"] = SameString($pageName, "ContextoView"); // If View page, no primary button
                    } else { // List page
    ...
    public function run()
    {
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->ano);
        $this->setupLookupOptions($this->revisao);
        $this->setupLookupOptions($this->usuario_idusuario);
        $this->setupLookupOptions($this->usuario_idusuario1);
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idcontexto") ?? Key(0) ?? Route(2)) !== null) {
                $this->idcontexto->setQueryStringValue($keyValue);
                $this->idcontexto->setOldValue($this->idcontexto->QueryStringValue);
            } elseif (Post("idcontexto") !== null) {
                $this->idcontexto->setFormValue(Post("idcontexto"));
                $this->idcontexto->setOldValue($this->idcontexto->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idcontexto") ?? Route("idcontexto")) !== null) {
                    $this->idcontexto->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->idcontexto->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ContextoList"); // No matching record, return to list
                        return;
                    }
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ContextoList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ContextoList") {
                            ...
                            $returnUrl = "ContextoList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idcontexto' first before field var 'x_idcontexto'
        $val = $CurrentForm->hasValue("idcontexto") ? $CurrentForm->getValue("idcontexto") : $CurrentForm->getValue("x_idcontexto");
        if (!$this->idcontexto->IsDetailKey) {
            $this->idcontexto->setFormValue($val);
        }

        // Check field name 'ano' first before field var 'x_ano'
        $val = $CurrentForm->hasValue("ano") ? $CurrentForm->getValue("ano") : $CurrentForm->getValue("x_ano");
        if (!$this->ano->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ano->Visible = false; // Disable update for API request
            } else {
                $this->ano->setFormValue($val);
            }
        }
        ...
        if (!$this->revisao->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->revisao->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'usuario_idusuario1' first before field var 'x_usuario_idusuario1'
        $val = $CurrentForm->hasValue("usuario_idusuario1") ? $CurrentForm->getValue("usuario_idusuario1") : $CurrentForm->getValue("x_usuario_idusuario1");
        if (!$this->usuario_idusuario1->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->usuario_idusuario1->Visible = false; // Disable update for API request
            } else {
                $this->usuario_idusuario1->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->idcontexto->CurrentValue = $this->idcontexto->FormValue;
        ...
        $this->usuario_idusuario1->CurrentValue = $this->usuario_idusuario1->FormValue;
    ...
    {
        $row = [];
        $row['idcontexto'] = $this->idcontexto->DefaultValue;
        $row['ano'] = $this->ano->DefaultValue;
        ...
        $row['usuario_idusuario1'] = $this->usuario_idusuario1->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        // idcontexto
        $this->idcontexto->RowCssClass = "row";

        // ano
        $this->ano->RowCssClass = "row";
        ...
        // usuario_idusuario1
        $this->usuario_idusuario1->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->usuario_idusuario1->CurrentValue));
            if ($curVal != "") {
                $this->usuario_idusuario1->ViewValue = $this->usuario_idusuario1->lookupCacheOption($curVal);
            } else {
                $this->usuario_idusuario1->ViewValue = $this->usuario_idusuario1->Lookup !== null && is_array($this->usuario_idusuario1->lookupOptions()) && count($this->usuario_idusuario1->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->usuario_idusuario1->ViewValue !== null) { // Load from cache
                $this->usuario_idusuario1->EditValue = array_values($this->usuario_idusuario1->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->usuario_idusuario1->Lookup->getTable()->Fields["idusuario"]->searchExpression(), "=", $this->usuario_idusuario1->CurrentValue, $this->usuario_idusuario1->Lookup->getTable()->Fields["idusuario"]->searchDataType(), "");
                }
                $sqlWrk = $this->usuario_idusuario1->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->usuario_idusuario1->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idcontexto->Visible && $this->idcontexto->Required) {
                if (!$this->idcontexto->IsDetailKey && EmptyValue($this->idcontexto->FormValue)) {
                    $this->idcontexto->addErrorMessage(str_replace("%s", $this->idcontexto->caption(), $this->idcontexto->RequiredErrorMessage));
                }
            }
            if ($this->ano->Visible && $this->ano->Required) {
                if ($this->ano->FormValue == "") {
                    $this->ano->addErrorMessage(str_replace("%s", $this->ano->caption(), $this->ano->RequiredErrorMessage));
                }
            }
            ...
            if (!CheckInteger($this->revisao->FormValue)) {
                $this->revisao->addErrorMessage($this->revisao->getErrorMessage(false));
            }
            ...
            if ($this->usuario_idusuario1->Visible && $this->usuario_idusuario1->Required) {
                if (!$this->usuario_idusuario1->IsDetailKey && EmptyValue($this->usuario_idusuario1->FormValue)) {
                    $this->usuario_idusuario1->addErrorMessage(str_replace("%s", $this->usuario_idusuario1->caption(), $this->usuario_idusuario1->RequiredErrorMessage));
                }
            }
        ...
        $detailPage = Container("AnaliseSwotGrid");
        if (in_array("analise_swot", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("AnaliseSwotGrid");
            if (in_array("analise_swot", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "analise_swot"); // Load user level of detail table
    ...
    protected function getEditRow($rsold)
    {
        ...
        // ano
        $this->ano->setDbValueDef($rsnew, $this->ano->CurrentValue, $this->ano->ReadOnly);
        ...
        // usuario_idusuario1
        $this->usuario_idusuario1->setDbValueDef($rsnew, $this->usuario_idusuario1->CurrentValue, $this->usuario_idusuario1->ReadOnly);
    ...
    {
        if (isset($row['ano'])) { // ano
            $this->ano->CurrentValue = $row['ano'];
        }
        ...
        if (isset($row['usuario_idusuario1'])) { // usuario_idusuario1
            $this->usuario_idusuario1->CurrentValue = $row['usuario_idusuario1'];
        }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("analise_swot", $detailTblVar)) {
                $detailPageObj = Container("AnaliseSwotGrid");
                if ($detailPageObj->DetailEdit) {
                    ...
                    $detailPageObj->contexto_idcontexto->IsDetailKey = true;
                    $detailPageObj->contexto_idcontexto->CurrentValue = $this->idcontexto->CurrentValue;
                    $detailPageObj->contexto_idcontexto->setSessionValue($detailPageObj->contexto_idcontexto->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ContextoList"), "", $this->TableVar, true);
        $pageId = "edit";

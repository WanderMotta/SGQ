 ...
 */
class DocumentoInterno extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "documento_interno";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // iddocumento_interno
        $this->iddocumento_interno = new DbField(
            $this, // Table
            'x_iddocumento_interno', // Variable name
            'iddocumento_interno', // Name
            '`iddocumento_interno`', // Expression
            '`iddocumento_interno`', // Basic search expression
            19, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`iddocumento_interno`', // Virtual expression
            false, // Is virtual
        ...
        $this->iddocumento_interno->InputTextType = "text";
        $this->iddocumento_interno->Raw = true;
        $this->iddocumento_interno->IsAutoIncrement = true; // Autoincrement field
        $this->iddocumento_interno->IsPrimaryKey = true; // Primary key field
        $this->iddocumento_interno->IsForeignKey = true; // Foreign key field
        $this->iddocumento_interno->Nullable = false; // NOT NULL field
        $this->iddocumento_interno->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->iddocumento_interno->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->restringir_acesso->addMethod("getDefault", fn() => "Nao");
        ...
        $this->restringir_acesso->Lookup = new Lookup($this->restringir_acesso, 'documento_interno', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "revisao_documento") {
            $detailUrl = Container("revisao_documento")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_iddocumento_interno", $this->iddocumento_interno->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "DocumentoInternoList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "documento_interno";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->iddocumento_interno->setDbValue($conn->lastInsertId());
            $rs['iddocumento_interno'] = $this->iddocumento_interno->DbValue;
        }
        return $result;
    }
    ...
    {
        // Cascade Update detail table 'revisao_documento'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['iddocumento_interno']) && $rsold['iddocumento_interno'] != $rs['iddocumento_interno'])) { // Update detail field 'documento_interno_iddocumento_interno'
            $cascadeUpdate = true;
            $rscascade['documento_interno_iddocumento_interno'] = $rs['iddocumento_interno'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("revisao_documento")->loadRs("`documento_interno_iddocumento_interno` = " . QuotedValue($rsold['iddocumento_interno'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'idrevisao_documento';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("revisao_documento")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("revisao_documento")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("revisao_documento")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
        ...
        if ($success) {
            if (!isset($rs['iddocumento_interno']) && !EmptyValue($this->iddocumento_interno->CurrentValue)) {
                $rs['iddocumento_interno'] = $this->iddocumento_interno->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('iddocumento_interno', $rs)) {
                AddFilter($where, QuotedName('iddocumento_interno', $this->Dbid) . '=' . QuotedValue($rs['iddocumento_interno'], $this->iddocumento_interno->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'revisao_documento'
        $dtlrows = Container("revisao_documento")->loadRs("`documento_interno_iddocumento_interno` = " . QuotedValue($rs['iddocumento_interno'], DataType::NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("revisao_documento")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("revisao_documento")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("revisao_documento")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
            try {
    ...
    {
        $keys = [];
        $val = $current ? $this->iddocumento_interno->CurrentValue : $this->iddocumento_interno->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->iddocumento_interno->CurrentValue = $keys[0];
            } else {
                $this->iddocumento_interno->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('iddocumento_interno', $row) ? $row['iddocumento_interno'] : null;
        } else {
            $val = !EmptyValue($this->iddocumento_interno->OldValue) && !$current ? $this->iddocumento_interno->OldValue : $this->iddocumento_interno->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@iddocumento_interno@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("DocumentoInternoList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "DocumentoInternoView" => $Language->phrase("View"),
            "DocumentoInternoEdit" => $Language->phrase("Edit"),
            "DocumentoInternoAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "DocumentoInternoView",
            Config("API_ADD_ACTION") => "DocumentoInternoAdd",
            Config("API_EDIT_ACTION") => "DocumentoInternoEdit",
            Config("API_DELETE_ACTION") => "DocumentoInternoDelete",
            Config("API_LIST_ACTION") => "DocumentoInternoList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DocumentoInternoView", $parm);
        } else {
            $url = $this->keyUrl("DocumentoInternoView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "DocumentoInternoAdd?" . $parm;
        } else {
            $url = "DocumentoInternoAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DocumentoInternoEdit", $parm);
        } else {
            $url = $this->keyUrl("DocumentoInternoEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("DocumentoInternoList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DocumentoInternoAdd", $parm);
        } else {
            $url = $this->keyUrl("DocumentoInternoAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("DocumentoInternoList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("DocumentoInternoDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"iddocumento_interno\":" . VarToJson($this->iddocumento_interno->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->iddocumento_interno->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->iddocumento_interno->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("iddocumento_interno") ?? Route("iddocumento_interno")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                ...
                $this->iddocumento_interno->OldValue = $key;
            }
    ...
    public function exportDocument($doc, $result, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->iddocumento_interno);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->iddocumento_interno);
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->iddocumento_interno, '');

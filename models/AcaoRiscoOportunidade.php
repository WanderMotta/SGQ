...
/**
 * Table class for acao_risco_oportunidade
 */
class AcaoRiscoOportunidade extends DbTable
{
    protected $SqlFrom = "";
    ...
    // Fields
    public $idacao_risco_oportunidade;
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "acao_risco_oportunidade";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->idacao_risco_oportunidade = new DbField(
            $this, // Table
            'x_idacao_risco_oportunidade', // Variable name
            'idacao_risco_oportunidade', // Name
            '`idacao_risco_oportunidade`', // Expression
            '`idacao_risco_oportunidade`', // Basic search expression
            19, // Type
            2, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`idacao_risco_oportunidade`', // Virtual expression
            false, // Is virtual
        ...
        $this->idacao_risco_oportunidade->InputTextType = "text";
        $this->idacao_risco_oportunidade->Raw = true;
        $this->idacao_risco_oportunidade->IsAutoIncrement = true; // Autoincrement field
        $this->idacao_risco_oportunidade->IsPrimaryKey = true; // Primary key field
        $this->idacao_risco_oportunidade->Nullable = false; // NOT NULL field
        $this->idacao_risco_oportunidade->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->idacao_risco_oportunidade->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['idacao_risco_oportunidade'] = &$this->idacao_risco_oportunidade;
        ...
        $this->tipo_risco_oportunidade_idtipo_risco_oportunidade->Lookup = new Lookup($this->tipo_risco_oportunidade_idtipo_risco_oportunidade, 'tipo_risco_oportunidade', false, 'idtipo_risco_oportunidade', ["tipo_risco_oportunidade","","",""], '', '', [], [], [], [], [], [], false, '', '', "`tipo_risco_oportunidade`");
        ...
        // acao
        $this->acao = new DbField(
            $this, // Table
            'x_acao', // Variable name
            'acao', // Name
            '`acao`', // Expression
            '`acao`', // Basic search expression
            200, // Type
            45, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`acao`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->acao->InputTextType = "text";
        $this->acao->Nullable = false; // NOT NULL field
        $this->acao->Required = true; // Required field
        $this->acao->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        $this->obs->Nullable = false; // NOT NULL field
        $this->obs->Required = true; // Required field
        $this->obs->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "acao_risco_oportunidade";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->idacao_risco_oportunidade->setDbValue($conn->lastInsertId());
            $rs['idacao_risco_oportunidade'] = $this->idacao_risco_oportunidade->DbValue;
        }
        return $result;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['idacao_risco_oportunidade']) && !EmptyValue($this->idacao_risco_oportunidade->CurrentValue)) {
                $rs['idacao_risco_oportunidade'] = $this->idacao_risco_oportunidade->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('idacao_risco_oportunidade', $rs)) {
                AddFilter($where, QuotedName('idacao_risco_oportunidade', $this->Dbid) . '=' . QuotedValue($rs['idacao_risco_oportunidade'], $this->idacao_risco_oportunidade->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->idacao_risco_oportunidade->DbValue = $row['idacao_risco_oportunidade'];
    ...
    {
        return "`idacao_risco_oportunidade` = @idacao_risco_oportunidade@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->idacao_risco_oportunidade->CurrentValue : $this->idacao_risco_oportunidade->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->idacao_risco_oportunidade->CurrentValue = $keys[0];
            } else {
                $this->idacao_risco_oportunidade->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('idacao_risco_oportunidade', $row) ? $row['idacao_risco_oportunidade'] : null;
        } else {
            $val = !EmptyValue($this->idacao_risco_oportunidade->OldValue) && !$current ? $this->idacao_risco_oportunidade->OldValue : $this->idacao_risco_oportunidade->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@idacao_risco_oportunidade@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AcaoRiscoOportunidadeList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AcaoRiscoOportunidadeView" => $Language->phrase("View"),
            "AcaoRiscoOportunidadeEdit" => $Language->phrase("Edit"),
            "AcaoRiscoOportunidadeAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "AcaoRiscoOportunidadeList";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AcaoRiscoOportunidadeView",
            Config("API_ADD_ACTION") => "AcaoRiscoOportunidadeAdd",
            Config("API_EDIT_ACTION") => "AcaoRiscoOportunidadeEdit",
            Config("API_DELETE_ACTION") => "AcaoRiscoOportunidadeDelete",
            Config("API_LIST_ACTION") => "AcaoRiscoOportunidadeList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AcaoRiscoOportunidadeView", $parm);
        } else {
            $url = $this->keyUrl("AcaoRiscoOportunidadeView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "AcaoRiscoOportunidadeAdd?" . $parm;
        } else {
            $url = "AcaoRiscoOportunidadeAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AcaoRiscoOportunidadeEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AcaoRiscoOportunidadeList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AcaoRiscoOportunidadeAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AcaoRiscoOportunidadeList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AcaoRiscoOportunidadeDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"idacao_risco_oportunidade\":" . VarToJson($this->idacao_risco_oportunidade->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->idacao_risco_oportunidade->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->idacao_risco_oportunidade->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("idacao_risco_oportunidade") ?? Route("idacao_risco_oportunidade")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->idacao_risco_oportunidade->CurrentValue = $key;
            } else {
                $this->idacao_risco_oportunidade->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "AcaoRiscoOportunidadeList";
    ...
    public function renderListRow()
    {
        ...
        $this->idacao_risco_oportunidade->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        $this->acao->EditValue = $this->acao->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->idacao_risco_oportunidade);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->idacao_risco_oportunidade);

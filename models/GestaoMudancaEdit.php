 ...
 * Page class
 */
class GestaoMudancaEdit extends GestaoMudanca
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GestaoMudancaEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "GestaoMudancaEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idgestao_mudanca") ?? Key(0) ?? Route(2)) !== null) {
                $this->idgestao_mudanca->setQueryStringValue($keyValue);
                $this->idgestao_mudanca->setOldValue($this->idgestao_mudanca->QueryStringValue);
            } elseif (Post("idgestao_mudanca") !== null) {
                $this->idgestao_mudanca->setFormValue(Post("idgestao_mudanca"));
                $this->idgestao_mudanca->setOldValue($this->idgestao_mudanca->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("idgestao_mudanca") ?? Route("idgestao_mudanca")) !== null) {
                    ...
                    $this->idgestao_mudanca->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idgestao_mudanca->IsDetailKey) {
            $this->idgestao_mudanca->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->idgestao_mudanca->CurrentValue = $this->idgestao_mudanca->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idgestao_mudanca->Visible && $this->idgestao_mudanca->Required) {
                if (!$this->idgestao_mudanca->IsDetailKey && EmptyValue($this->idgestao_mudanca->FormValue)) {
                    $this->idgestao_mudanca->addErrorMessage(str_replace("%s", $this->idgestao_mudanca->caption(), $this->idgestao_mudanca->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // dt_inicio
        $this->dt_inicio->setDbValueDef($rsnew, UnFormatDateTime($this->dt_inicio->CurrentValue, $this->dt_inicio->formatPattern()), $this->dt_inicio->ReadOnly);

        // detalhamento
        $this->detalhamento->setDbValueDef($rsnew, $this->detalhamento->CurrentValue, $this->detalhamento->ReadOnly);
        ...
        // recursos
        $this->recursos->setDbValueDef($rsnew, $this->recursos->CurrentValue, $this->recursos->ReadOnly);

        // prazo_ate
        $this->prazo_ate->setDbValueDef($rsnew, UnFormatDateTime($this->prazo_ate->CurrentValue, $this->prazo_ate->formatPattern()), $this->prazo_ate->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['dt_inicio'])) { // dt_inicio
            $this->dt_inicio->CurrentValue = $row['dt_inicio'];
        }
        if (isset($row['detalhamento'])) { // detalhamento
            $this->detalhamento->CurrentValue = $row['detalhamento'];
        }
        ...
        if (isset($row['recursos'])) { // recursos
            $this->recursos->CurrentValue = $row['recursos'];
        }
        if (isset($row['prazo_ate'])) { // prazo_ate
            $this->prazo_ate->CurrentValue = $row['prazo_ate'];
        }

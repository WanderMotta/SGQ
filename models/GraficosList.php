 ...
 * Page class
 */
class GraficosList extends Graficos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GraficosList";
    ...
    public $FormName = "fgraficoslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "GraficosList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "GraficosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "GraficosDelete";
        $this->MultiUpdateUrl = "GraficosUpdate";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "indicadores") {
            $masterTbl = Container("indicadores");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("IndicadoresList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idgraficos->CurrentValue;
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_competencia_idcompetencia") &&
            $CurrentForm->hasValue("o_competencia_idcompetencia") &&
            $this->competencia_idcompetencia->CurrentValue != $this->competencia_idcompetencia->DefaultValue &&
            !($this->competencia_idcompetencia->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->competencia_idcompetencia->CurrentValue == $this->competencia_idcompetencia->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_indicadores_idindicadores") &&
            $CurrentForm->hasValue("o_indicadores_idindicadores") &&
            $this->indicadores_idindicadores->CurrentValue != $this->indicadores_idindicadores->DefaultValue &&
            !($this->indicadores_idindicadores->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->indicadores_idindicadores->CurrentValue == $this->indicadores_idindicadores->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->idgraficos->AdvancedSearch->toJson(), ","); // Field idgraficos
        $filterList = Concat($filterList, $this->competencia_idcompetencia->AdvancedSearch->toJson(), ","); // Field competencia_idcompetencia
        $filterList = Concat($filterList, $this->indicadores_idindicadores->AdvancedSearch->toJson(), ","); // Field indicadores_idindicadores
        $filterList = Concat($filterList, $this->data_base->AdvancedSearch->toJson(), ","); // Field data_base
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fgraficossrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idgraficos
        $this->idgraficos->AdvancedSearch->SearchValue = @$filter["x_idgraficos"];
        $this->idgraficos->AdvancedSearch->SearchOperator = @$filter["z_idgraficos"];
        $this->idgraficos->AdvancedSearch->SearchCondition = @$filter["v_idgraficos"];
        $this->idgraficos->AdvancedSearch->SearchValue2 = @$filter["y_idgraficos"];
        $this->idgraficos->AdvancedSearch->SearchOperator2 = @$filter["w_idgraficos"];
        $this->idgraficos->AdvancedSearch->save();

        // Field competencia_idcompetencia
        $this->competencia_idcompetencia->AdvancedSearch->SearchValue = @$filter["x_competencia_idcompetencia"];
        $this->competencia_idcompetencia->AdvancedSearch->SearchOperator = @$filter["z_competencia_idcompetencia"];
        $this->competencia_idcompetencia->AdvancedSearch->SearchCondition = @$filter["v_competencia_idcompetencia"];
        $this->competencia_idcompetencia->AdvancedSearch->SearchValue2 = @$filter["y_competencia_idcompetencia"];
        $this->competencia_idcompetencia->AdvancedSearch->SearchOperator2 = @$filter["w_competencia_idcompetencia"];
        $this->competencia_idcompetencia->AdvancedSearch->save();

        // Field indicadores_idindicadores
        $this->indicadores_idindicadores->AdvancedSearch->SearchValue = @$filter["x_indicadores_idindicadores"];
        $this->indicadores_idindicadores->AdvancedSearch->SearchOperator = @$filter["z_indicadores_idindicadores"];
        $this->indicadores_idindicadores->AdvancedSearch->SearchCondition = @$filter["v_indicadores_idindicadores"];
        $this->indicadores_idindicadores->AdvancedSearch->SearchValue2 = @$filter["y_indicadores_idindicadores"];
        $this->indicadores_idindicadores->AdvancedSearch->SearchOperator2 = @$filter["w_indicadores_idindicadores"];
        $this->indicadores_idindicadores->AdvancedSearch->save();

        // Field data_base
        $this->data_base->AdvancedSearch->SearchValue = @$filter["x_data_base"];
        $this->data_base->AdvancedSearch->SearchOperator = @$filter["z_data_base"];
        $this->data_base->AdvancedSearch->SearchCondition = @$filter["v_data_base"];
        $this->data_base->AdvancedSearch->SearchValue2 = @$filter["y_data_base"];
        $this->data_base->AdvancedSearch->SearchOperator2 = @$filter["w_data_base"];
        $this->data_base->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->idgraficos, $default, false); // idgraficos
        $this->buildSearchSql($where, $this->competencia_idcompetencia, $default, false); // competencia_idcompetencia
        $this->buildSearchSql($where, $this->indicadores_idindicadores, $default, false); // indicadores_idindicadores
        $this->buildSearchSql($where, $this->data_base, $default, false); // data_base
        ...
        if (!$default && $this->Command == "search") {
            $this->idgraficos->AdvancedSearch->save(); // idgraficos
            $this->competencia_idcompetencia->AdvancedSearch->save(); // competencia_idcompetencia
            $this->indicadores_idindicadores->AdvancedSearch->save(); // indicadores_idindicadores
            $this->data_base->AdvancedSearch->save(); // data_base
    ...
    public function showFilterList()
    {
        ...
        // Field competencia_idcompetencia
        $filter = $this->queryBuilderWhere("competencia_idcompetencia");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->competencia_idcompetencia, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->competencia_idcompetencia->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field indicadores_idindicadores
        $filter = $this->queryBuilderWhere("indicadores_idindicadores");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->indicadores_idindicadores, false, false);
        }
        if ($filter != "") {
        ...
        // Field obs
        $filter = $this->queryBuilderWhere("obs");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->obs, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->obs->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    {
        if ($this->idgraficos->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->competencia_idcompetencia->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->indicadores_idindicadores->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->data_base->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->idgraficos->AdvancedSearch->unsetSession();
        $this->competencia_idcompetencia->AdvancedSearch->unsetSession();
        $this->indicadores_idindicadores->AdvancedSearch->unsetSession();
        $this->data_base->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->idgraficos->AdvancedSearch->load();
        $this->competencia_idcompetencia->AdvancedSearch->load();
        $this->indicadores_idindicadores->AdvancedSearch->load();
        $this->data_base->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->competencia_idcompetencia->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->competencia_idcompetencia, $ctrl); // competencia_idcompetencia
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->idgraficos->setSort("");
                $this->competencia_idcompetencia->setSort("");
                $this->indicadores_idindicadores->setSort("");
                $this->data_base->setSort("");
                $this->valor->setSort("");
                $this->obs->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"graficos\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"graficos\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"graficos\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fgraficoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fgraficoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->idgraficos->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"graficos\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "competencia_idcompetencia");
            $this->createColumnOption($option, "indicadores_idindicadores");
            $this->createColumnOption($option, "valor");
            $this->createColumnOption($option, "obs");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fgraficossrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fgraficossrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fgraficoslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fgraficoslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fgraficoslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_graficos", "data-rowtype" => RowType::ADD]);
    ...
    protected function loadSearchValues()
    {
        ...
        // idgraficos
        if ($this->idgraficos->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->idgraficos->AdvancedSearch->SearchValue != "" || $this->idgraficos->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // competencia_idcompetencia
        if ($this->competencia_idcompetencia->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->competencia_idcompetencia->AdvancedSearch->SearchValue != "" || $this->competencia_idcompetencia->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // indicadores_idindicadores
        if ($this->indicadores_idindicadores->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->indicadores_idindicadores->AdvancedSearch->SearchValue != "" || $this->indicadores_idindicadores->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // data_base
        if ($this->data_base->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->data_base->AdvancedSearch->SearchValue != "" || $this->data_base->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_competencia_idcompetencia")) {
            $this->competencia_idcompetencia->setOldValue($CurrentForm->getValue("o_competencia_idcompetencia"));
        }
        ...
        if ($CurrentForm->hasValue("o_indicadores_idindicadores")) {
            $this->indicadores_idindicadores->setOldValue($CurrentForm->getValue("o_indicadores_idindicadores"));
        }
        ...
        if (!$this->idgraficos->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->isExport()) {
                $this->obs->ViewValue = $this->highlightValue($this->obs);
            }
            ...
            if ($this->indicadores_idindicadores->getSessionValue() != "") {
                ...
                $this->indicadores_idindicadores->OldValue = $this->indicadores_idindicadores->CurrentValue;
                $curVal = strval($this->indicadores_idindicadores->CurrentValue);
                if ($curVal != "") {
                    $this->indicadores_idindicadores->ViewValue = $this->indicadores_idindicadores->lookupCacheOption($curVal);
                    if ($this->indicadores_idindicadores->ViewValue === null) { // Lookup from database
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['competencia_idcompetencia']); // competencia_idcompetencia
        $hash .= GetFieldHash($row['indicadores_idindicadores']); // indicadores_idindicadores
        $hash .= GetFieldHash($row['valor']); // valor
        $hash .= GetFieldHash($row['obs']); // obs
        return md5($hash);
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fgraficoslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-table=\"graficos\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" data-ew-action=\"modal\" data-url=\"GraficosQuery\" data-btn=\"SearchBtn\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" href=\"GraficosQuery\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        }
        $item->Visible = true;

        // Search highlight button
        $item = &$this->SearchOptions->add("searchhighlight");
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"fgraficossrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "indicadores") {
            $indicadores = new IndicadoresList();
            $rsmaster = $indicadores->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($indicadores);
                    $indicadores->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }

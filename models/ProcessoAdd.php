 ...
 * Page class
 */
class ProcessoAdd extends Processo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProcessoAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ProcessoAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ProcessoView") {
    ...
    {
        $this->tipo_idtipo->DefaultValue = $this->tipo_idtipo->getDefault(); // PHP
        $this->tipo_idtipo->OldValue = $this->tipo_idtipo->DefaultValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!Config("CREATE_UPLOAD_FILE_ON_COPY")) {
                $this->proced_int_trabalho->Upload->DbValue = null;
            }
            ...
            if (!Config("CREATE_UPLOAD_FILE_ON_COPY")) {
                $this->formulario->Upload->DbValue = null;
            }
            if ($this->isShow() || $this->isCopy()) {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("processo_indicadores", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->proced_int_trabalho->Visible && !$this->proced_int_trabalho->Upload->KeepFile) {
            if (!EmptyValue($this->proced_int_trabalho->Upload->FileName)) {
                ...
                $this->proced_int_trabalho->setDbValueDef($rsnew, $this->proced_int_trabalho->Upload->FileName, false);
            }
        }
        if ($this->mapa->Visible && !$this->mapa->Upload->KeepFile) {
            if (!EmptyValue($this->mapa->Upload->FileName)) {
                ...
                $this->mapa->setDbValueDef($rsnew, $this->mapa->Upload->FileName, false);
            }
        }
        if ($this->formulario->Visible && !$this->formulario->Upload->KeepFile) {
            if (!EmptyValue($this->formulario->Upload->FileName)) {
                ...
                $this->formulario->setDbValueDef($rsnew, $this->formulario->Upload->FileName, false);
            }
        }

        // Update current values
        $this->setCurrentValues($rsnew);
        if ($this->processo->CurrentValue != "") { // Check field with unique index
            $filter = "(`processo` = '" . AdjustSql($this->processo->CurrentValue, $this->Dbid) . "')";
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ProcessoIndicadoresGrid");
            if (in_array("processo_indicadores", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->processo_idprocesso->setSessionValue($this->idprocesso->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "processo_indicadores"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->processo_idprocesso->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // revisao
        $this->revisao->setDbValueDef($rsnew, $this->revisao->CurrentValue, false);

        // tipo_idtipo
        $this->tipo_idtipo->setDbValueDef($rsnew, $this->tipo_idtipo->CurrentValue, strval($this->tipo_idtipo->CurrentValue) == "");

        // processo
        $this->processo->setDbValueDef($rsnew, $this->processo->CurrentValue, false);

        // responsaveis
        $this->responsaveis->setDbValueDef($rsnew, $this->responsaveis->CurrentValue, false);

        // objetivo
        $this->objetivo->setDbValueDef($rsnew, $this->objetivo->CurrentValue, false);

        // proc_antes
        $this->proc_antes->setDbValueDef($rsnew, $this->proc_antes->CurrentValue, false);

        // proc_depois
        $this->proc_depois->setDbValueDef($rsnew, $this->proc_depois->CurrentValue, false);

        // eqpto_recursos
        $this->eqpto_recursos->setDbValueDef($rsnew, $this->eqpto_recursos->CurrentValue, false);

        // entradas
        $this->entradas->setDbValueDef($rsnew, $this->entradas->CurrentValue, false);

        // atividade_principal
        $this->atividade_principal->setDbValueDef($rsnew, $this->atividade_principal->CurrentValue, false);

        // saidas_resultados
        $this->saidas_resultados->setDbValueDef($rsnew, $this->saidas_resultados->CurrentValue, false);

        // requsito_saidas
        $this->requsito_saidas->setDbValueDef($rsnew, $this->requsito_saidas->CurrentValue, false);

        // riscos
        $this->riscos->setDbValueDef($rsnew, $this->riscos->CurrentValue, false);

        // oportunidades
        $this->oportunidades->setDbValueDef($rsnew, $this->oportunidades->CurrentValue, false);

        // propriedade_externa
        $this->propriedade_externa->setDbValueDef($rsnew, $this->propriedade_externa->CurrentValue, false);

        // conhecimentos
        $this->conhecimentos->setDbValueDef($rsnew, $this->conhecimentos->CurrentValue, false);

        // documentos_aplicados
        $this->documentos_aplicados->setDbValueDef($rsnew, $this->documentos_aplicados->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['tipo_idtipo'])) { // tipo_idtipo
            $this->tipo_idtipo->setFormValue($row['tipo_idtipo']);
        }
        if (isset($row['processo'])) { // processo
            $this->processo->setFormValue($row['processo']);
        }
        if (isset($row['responsaveis'])) { // responsaveis
            $this->responsaveis->setFormValue($row['responsaveis']);
        }
        if (isset($row['objetivo'])) { // objetivo
            $this->objetivo->setFormValue($row['objetivo']);
        }
        if (isset($row['proc_antes'])) { // proc_antes
            $this->proc_antes->setFormValue($row['proc_antes']);
        }
        if (isset($row['proc_depois'])) { // proc_depois
            $this->proc_depois->setFormValue($row['proc_depois']);
        }
        if (isset($row['eqpto_recursos'])) { // eqpto_recursos
            $this->eqpto_recursos->setFormValue($row['eqpto_recursos']);
        }
        if (isset($row['entradas'])) { // entradas
            $this->entradas->setFormValue($row['entradas']);
        }
        if (isset($row['atividade_principal'])) { // atividade_principal
            $this->atividade_principal->setFormValue($row['atividade_principal']);
        }
        if (isset($row['saidas_resultados'])) { // saidas_resultados
            $this->saidas_resultados->setFormValue($row['saidas_resultados']);
        }
        if (isset($row['requsito_saidas'])) { // requsito_saidas
            $this->requsito_saidas->setFormValue($row['requsito_saidas']);
        }
        if (isset($row['riscos'])) { // riscos
            $this->riscos->setFormValue($row['riscos']);
        }
        if (isset($row['oportunidades'])) { // oportunidades
            $this->oportunidades->setFormValue($row['oportunidades']);
        }
        if (isset($row['propriedade_externa'])) { // propriedade_externa
            $this->propriedade_externa->setFormValue($row['propriedade_externa']);
        }
        if (isset($row['conhecimentos'])) { // conhecimentos
            $this->conhecimentos->setFormValue($row['conhecimentos']);
        }
        if (isset($row['documentos_aplicados'])) { // documentos_aplicados
            $this->documentos_aplicados->setFormValue($row['documentos_aplicados']);
        }
        if (isset($row['proced_int_trabalho'])) { // proced_int_trabalho
            $this->proced_int_trabalho->setFormValue($row['proced_int_trabalho']);
        }
        if (isset($row['mapa'])) { // mapa
            $this->mapa->setFormValue($row['mapa']);
        }
        if (isset($row['formulario'])) { // formulario
            $this->formulario->setFormValue($row['formulario']);
        }
    }

 ...
 * Page class
 */
class AnaliseCriticaDirecaoAdd extends AnaliseCriticaDirecao
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AnaliseCriticaDirecaoAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "AnaliseCriticaDirecaoAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "AnaliseCriticaDirecaoView") {
    ...
    protected function getAddRow()
    {
        ...
        // participantes
        $this->participantes->setDbValueDef($rsnew, $this->participantes->CurrentValue, false);

        // usuario_idusuario
        $this->usuario_idusuario->setDbValueDef($rsnew, $this->usuario_idusuario->CurrentValue, false);

        // situacao_anterior
        $this->situacao_anterior->setDbValueDef($rsnew, $this->situacao_anterior->CurrentValue, false);

        // mudanca_sqg
        $this->mudanca_sqg->setDbValueDef($rsnew, $this->mudanca_sqg->CurrentValue, false);

        // desempenho_eficacia
        $this->desempenho_eficacia->setDbValueDef($rsnew, $this->desempenho_eficacia->CurrentValue, false);

        // satisfacao_cliente
        $this->satisfacao_cliente->setDbValueDef($rsnew, $this->satisfacao_cliente->CurrentValue, false);

        // objetivos_alcançados
        $this->objetivos_alcancados->setDbValueDef($rsnew, $this->objetivos_alcancados->CurrentValue, false);

        // desempenho_processo
        $this->desempenho_processo->setDbValueDef($rsnew, $this->desempenho_processo->CurrentValue, false);

        // nao_confomidade_acoes_corretivas
        $this->nao_confomidade_acoes_corretivas->setDbValueDef($rsnew, $this->nao_confomidade_acoes_corretivas->CurrentValue, false);

        // monitoramento_medicao
        $this->monitoramento_medicao->setDbValueDef($rsnew, $this->monitoramento_medicao->CurrentValue, false);

        // resultado_auditoria
        $this->resultado_auditoria->setDbValueDef($rsnew, $this->resultado_auditoria->CurrentValue, false);

        // desempenho_provedores_ext
        $this->desempenho_provedores_ext->setDbValueDef($rsnew, $this->desempenho_provedores_ext->CurrentValue, false);

        // suficiencia_recursos
        $this->suficiencia_recursos->setDbValueDef($rsnew, $this->suficiencia_recursos->CurrentValue, false);

        // acoes_risco_oportunidades
        $this->acoes_risco_oportunidades->setDbValueDef($rsnew, $this->acoes_risco_oportunidades->CurrentValue, false);

        // oportunidade_melhora_entrada
        $this->oportunidade_melhora_entrada->setDbValueDef($rsnew, $this->oportunidade_melhora_entrada->CurrentValue, false);

        // oportunidade_melhora_saida
        $this->oportunidade_melhora_saida->setDbValueDef($rsnew, $this->oportunidade_melhora_saida->CurrentValue, false);

        // qualquer_mudanca_sgq
        $this->qualquer_mudanca_sgq->setDbValueDef($rsnew, $this->qualquer_mudanca_sgq->CurrentValue, false);

        // nec_recurso
        $this->nec_recurso->setDbValueDef($rsnew, $this->nec_recurso->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['participantes'])) { // participantes
            $this->participantes->setFormValue($row['participantes']);
        }
        ...
        if (isset($row['situacao_anterior'])) { // situacao_anterior
            $this->situacao_anterior->setFormValue($row['situacao_anterior']);
        }
        if (isset($row['mudanca_sqg'])) { // mudanca_sqg
            $this->mudanca_sqg->setFormValue($row['mudanca_sqg']);
        }
        if (isset($row['desempenho_eficacia'])) { // desempenho_eficacia
            $this->desempenho_eficacia->setFormValue($row['desempenho_eficacia']);
        }
        if (isset($row['satisfacao_cliente'])) { // satisfacao_cliente
            $this->satisfacao_cliente->setFormValue($row['satisfacao_cliente']);
        }
        if (isset($row['objetivos_alcançados'])) { // objetivos_alcançados
            $this->objetivos_alcancados->setFormValue($row['objetivos_alcançados']);
        }
        if (isset($row['desempenho_processo'])) { // desempenho_processo
            $this->desempenho_processo->setFormValue($row['desempenho_processo']);
        }
        if (isset($row['nao_confomidade_acoes_corretivas'])) { // nao_confomidade_acoes_corretivas
            $this->nao_confomidade_acoes_corretivas->setFormValue($row['nao_confomidade_acoes_corretivas']);
        }
        if (isset($row['monitoramento_medicao'])) { // monitoramento_medicao
            $this->monitoramento_medicao->setFormValue($row['monitoramento_medicao']);
        }
        if (isset($row['resultado_auditoria'])) { // resultado_auditoria
            $this->resultado_auditoria->setFormValue($row['resultado_auditoria']);
        }
        if (isset($row['desempenho_provedores_ext'])) { // desempenho_provedores_ext
            $this->desempenho_provedores_ext->setFormValue($row['desempenho_provedores_ext']);
        }
        if (isset($row['suficiencia_recursos'])) { // suficiencia_recursos
            $this->suficiencia_recursos->setFormValue($row['suficiencia_recursos']);
        }
        if (isset($row['acoes_risco_oportunidades'])) { // acoes_risco_oportunidades
            $this->acoes_risco_oportunidades->setFormValue($row['acoes_risco_oportunidades']);
        }
        if (isset($row['oportunidade_melhora_entrada'])) { // oportunidade_melhora_entrada
            $this->oportunidade_melhora_entrada->setFormValue($row['oportunidade_melhora_entrada']);
        }
        if (isset($row['oportunidade_melhora_saida'])) { // oportunidade_melhora_saida
            $this->oportunidade_melhora_saida->setFormValue($row['oportunidade_melhora_saida']);
        }
        if (isset($row['qualquer_mudanca_sgq'])) { // qualquer_mudanca_sgq
            $this->qualquer_mudanca_sgq->setFormValue($row['qualquer_mudanca_sgq']);
        }
        if (isset($row['nec_recurso'])) { // nec_recurso
            $this->nec_recurso->setFormValue($row['nec_recurso']);
        }

 ...
 * Page class
 */
class DocumentoInternoView extends DocumentoInterno
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentoInternoView";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentoInternoView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("iddocumento_interno") ?? Route("iddocumento_interno")) !== null) {
            $this->RecKey["iddocumento_interno"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        } elseif (Post("iddocumento_interno") !== null) {
            $this->iddocumento_interno->setFormValue(Post("iddocumento_interno"));
            $this->RecKey["iddocumento_interno"] = $this->iddocumento_interno->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->iddocumento_interno->setQueryStringValue($keyValue);
            $this->RecKey["iddocumento_interno"] = $this->iddocumento_interno->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "DocumentoInternoList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("DocumentoInternoList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->iddocumento_interno->CurrentValue, $this->CurrentRow['iddocumento_interno'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "DocumentoInternoList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_revisao_documento");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("revisao_documento", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("RevisaoDocumentoList?" . Config("TABLE_SHOW_MASTER") . "=documento_interno&" . GetForeignKeyUrl("fk_iddocumento_interno", $this->iddocumento_interno->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("RevisaoDocumentoGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'documento_interno')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=revisao_documento"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "revisao_documento";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'documento_interno')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=revisao_documento"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "revisao_documento";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'documento_interno')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=revisao_documento"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (revisao_documento)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("revisao_documento", explode(",", $this->getCurrentDetailTable()))) {
            $revisao_documento = new RevisaoDocumentoList();
            $rsdetail = $revisao_documento->loadRs($revisao_documento->getDetailFilterFromSession(), $revisao_documento->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($revisao_documento);
                    $revisao_documento->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }

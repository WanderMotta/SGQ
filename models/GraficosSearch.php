...
class GraficosSearch extends Graficos
{
    ...
    // Page object name
    public $PageObjName = "GraficosSearch";
    ...
    // CSS class/style
    public $CurrentPageName = "GraficosSearch";
    ...
    {
        $srchUrl = "";
        $this->buildSearchUrl($srchUrl, $this->idgraficos); // idgraficos
        $this->buildSearchUrl($srchUrl, $this->competencia_idcompetencia); // competencia_idcompetencia
        $this->buildSearchUrl($srchUrl, $this->indicadores_idindicadores); // indicadores_idindicadores
        $this->buildSearchUrl($srchUrl, $this->data_base); // data_base
        $this->buildSearchUrl($srchUrl, $this->valor); // valor
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->idgraficos->EditValue = $this->idgraficos->AdvancedSearch->SearchValue;
            $this->idgraficos->PlaceHolder = RemoveHtml($this->idgraficos->caption());

            // competencia_idcompetencia
            $this->competencia_idcompetencia->setupEditAttributes();
            $curVal = trim(strval($this->competencia_idcompetencia->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->competencia_idcompetencia->AdvancedSearch->ViewValue = $this->competencia_idcompetencia->lookupCacheOption($curVal);
            } else {
                $this->competencia_idcompetencia->AdvancedSearch->ViewValue = $this->competencia_idcompetencia->Lookup !== null && is_array($this->competencia_idcompetencia->lookupOptions()) && count($this->competencia_idcompetencia->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->competencia_idcompetencia->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->competencia_idcompetencia->EditValue = array_values($this->competencia_idcompetencia->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->competencia_idcompetencia->Lookup->getTable()->Fields["idcompetencia"]->searchExpression(), "=", $this->competencia_idcompetencia->AdvancedSearch->SearchValue, $this->competencia_idcompetencia->Lookup->getTable()->Fields["idcompetencia"]->searchDataType(), "");
                }
            ...
            $this->data_base->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->data_base->AdvancedSearch->SearchValue, $this->data_base->formatPattern()), $this->data_base->formatPattern()));
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->idgraficos->AdvancedSearch->SearchValue)) {
            $this->idgraficos->addErrorMessage($this->idgraficos->getErrorMessage(false));
        }
        if (!CheckDate($this->data_base->AdvancedSearch->SearchValue, $this->data_base->formatPattern())) {
            $this->data_base->addErrorMessage($this->data_base->getErrorMessage(false));
        }
        if (!CheckNumber($this->valor->AdvancedSearch->SearchValue)) {

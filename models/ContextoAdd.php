 ...
 * Page class
 */
class ContextoAdd extends Contexto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContextoAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ContextoAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ContextoView") {
    ...
    {
        $this->ano->DefaultValue = $this->ano->getDefault(); // PHP
        $this->ano->OldValue = $this->ano->DefaultValue;
        $this->revisao->DefaultValue = $this->revisao->getDefault(); // PHP
        $this->revisao->OldValue = $this->revisao->DefaultValue;
    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("analise_swot", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("AnaliseSwotGrid");
            if (in_array("analise_swot", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->contexto_idcontexto->setSessionValue($this->idcontexto->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "analise_swot"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->contexto_idcontexto->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // ano
        $this->ano->setDbValueDef($rsnew, $this->ano->CurrentValue, strval($this->ano->CurrentValue) == "");

        // revisao
        $this->revisao->setDbValueDef($rsnew, $this->revisao->CurrentValue, strval($this->revisao->CurrentValue) == "");
        ...
        // usuario_idusuario1
        $this->usuario_idusuario1->setDbValueDef($rsnew, $this->usuario_idusuario1->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['ano'])) { // ano
            $this->ano->setFormValue($row['ano']);
        }
        if (isset($row['revisao'])) { // revisao
            $this->revisao->setFormValue($row['revisao']);
        }
        ...
        if (isset($row['usuario_idusuario1'])) { // usuario_idusuario1
            $this->usuario_idusuario1->setFormValue($row['usuario_idusuario1']);
        }

 ...
 * Page class
 */
class ObjetivoQualidadeEdit extends ObjetivoQualidade
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ObjetivoQualidadeEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ObjetivoQualidadeEdit";
    ...
    {
        $this->idobjetivo_qualidade->setVisibility();
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idobjetivo_qualidade") ?? Key(0) ?? Route(2)) !== null) {
                $this->idobjetivo_qualidade->setQueryStringValue($keyValue);
                $this->idobjetivo_qualidade->setOldValue($this->idobjetivo_qualidade->QueryStringValue);
            } elseif (Post("idobjetivo_qualidade") !== null) {
                $this->idobjetivo_qualidade->setFormValue(Post("idobjetivo_qualidade"));
                $this->idobjetivo_qualidade->setOldValue($this->idobjetivo_qualidade->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idobjetivo_qualidade") ?? Route("idobjetivo_qualidade")) !== null) {
                    $this->idobjetivo_qualidade->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->idobjetivo_qualidade->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ObjetivoQualidadeList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ObjetivoQualidadeList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ObjetivoQualidadeList") {
                            ...
                            $returnUrl = "ObjetivoQualidadeList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idobjetivo_qualidade' first before field var 'x_idobjetivo_qualidade'
        $val = $CurrentForm->hasValue("idobjetivo_qualidade") ? $CurrentForm->getValue("idobjetivo_qualidade") : $CurrentForm->getValue("x_idobjetivo_qualidade");
        if (!$this->idobjetivo_qualidade->IsDetailKey) {
            $this->idobjetivo_qualidade->setFormValue($val);
        }
        ...
        // Check field name 'como_medir' first before field var 'x_como_medir'
        $val = $CurrentForm->hasValue("como_medir") ? $CurrentForm->getValue("como_medir") : $CurrentForm->getValue("x_como_medir");
        if (!$this->como_medir->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->como_medir->Visible = false; // Disable update for API request
            } else {
                $this->como_medir->setFormValue($val);
            }
        }
        ...
        // Check field name 'como_avaliar' first before field var 'x_como_avaliar'
        $val = $CurrentForm->hasValue("como_avaliar") ? $CurrentForm->getValue("como_avaliar") : $CurrentForm->getValue("x_como_avaliar");
        if (!$this->como_avaliar->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->como_avaliar->Visible = false; // Disable update for API request
            } else {
                $this->como_avaliar->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->idobjetivo_qualidade->CurrentValue = $this->idobjetivo_qualidade->FormValue;
        ...
        $this->como_medir->CurrentValue = $this->como_medir->FormValue;
        $this->o_q_sera_feito->CurrentValue = $this->o_q_sera_feito->FormValue;
        $this->como_avaliar->CurrentValue = $this->como_avaliar->FormValue;
    ...
    public function renderRow()
    {
        ...
        $this->idobjetivo_qualidade->RowCssClass = "row";
        ...
        // como_medir
        $this->como_medir->RowCssClass = "row";

        // o_q_sera_feito
        $this->o_q_sera_feito->RowCssClass = "row";

        // como_avaliar
        $this->como_avaliar->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->como_medir->EditValue = HtmlEncode($this->como_medir->CurrentValue);
            ...
            $this->como_avaliar->EditValue = HtmlEncode($this->como_avaliar->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idobjetivo_qualidade->Visible && $this->idobjetivo_qualidade->Required) {
                if (!$this->idobjetivo_qualidade->IsDetailKey && EmptyValue($this->idobjetivo_qualidade->FormValue)) {
                    $this->idobjetivo_qualidade->addErrorMessage(str_replace("%s", $this->idobjetivo_qualidade->caption(), $this->idobjetivo_qualidade->RequiredErrorMessage));
                }
            }
            ...
            if ($this->como_medir->Visible && $this->como_medir->Required) {
                if (!$this->como_medir->IsDetailKey && EmptyValue($this->como_medir->FormValue)) {
                    $this->como_medir->addErrorMessage(str_replace("%s", $this->como_medir->caption(), $this->como_medir->RequiredErrorMessage));
                }
            }
            ...
            if ($this->como_avaliar->Visible && $this->como_avaliar->Required) {
                if (!$this->como_avaliar->IsDetailKey && EmptyValue($this->como_avaliar->FormValue)) {
                    $this->como_avaliar->addErrorMessage(str_replace("%s", $this->como_avaliar->caption(), $this->como_avaliar->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // como_medir
        $this->como_medir->setDbValueDef($rsnew, $this->como_medir->CurrentValue, $this->como_medir->ReadOnly);

        // o_q_sera_feito
        $this->o_q_sera_feito->setDbValueDef($rsnew, $this->o_q_sera_feito->CurrentValue, $this->o_q_sera_feito->ReadOnly);

        // como_avaliar
        $this->como_avaliar->setDbValueDef($rsnew, $this->como_avaliar->CurrentValue, $this->como_avaliar->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['como_medir'])) { // como_medir
            $this->como_medir->CurrentValue = $row['como_medir'];
        }
        if (isset($row['o_q_sera_feito'])) { // o_q_sera_feito
            $this->o_q_sera_feito->CurrentValue = $row['o_q_sera_feito'];
        }
        if (isset($row['como_avaliar'])) { // como_avaliar
            $this->como_avaliar->CurrentValue = $row['como_avaliar'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ObjetivoQualidadeList"), "", $this->TableVar, true);
        $pageId = "edit";

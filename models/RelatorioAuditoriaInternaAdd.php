 ...
 * Page class
 */
class RelatorioAuditoriaInternaAdd extends RelatorioAuditoriaInterna
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RelatorioAuditoriaInternaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "RelatorioAuditoriaInternaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "RelatorioAuditoriaInternaView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("item_rel_aud_interna", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ItemRelAudInternaGrid");
            if (in_array("item_rel_aud_interna", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->relatorio_auditoria_interna_idrelatorio_auditoria_interna->setSessionValue($this->idrelatorio_auditoria_interna->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "item_rel_aud_interna"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->relatorio_auditoria_interna_idrelatorio_auditoria_interna->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // auditor
        $this->auditor->setDbValueDef($rsnew, $this->auditor->CurrentValue, false);

        // aprovador
        $this->aprovador->setDbValueDef($rsnew, $this->aprovador->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['auditor'])) { // auditor
            $this->auditor->setFormValue($row['auditor']);
        }
        if (isset($row['aprovador'])) { // aprovador
            $this->aprovador->setFormValue($row['aprovador']);
        }
    }

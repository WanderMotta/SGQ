 ...
 * Page class
 */
class ImpactoEdit extends Impacto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ImpactoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ImpactoEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idimpacto") ?? Key(0) ?? Route(2)) !== null) {
                $this->idimpacto->setQueryStringValue($keyValue);
                $this->idimpacto->setOldValue($this->idimpacto->QueryStringValue);
            } elseif (Post("idimpacto") !== null) {
                $this->idimpacto->setFormValue(Post("idimpacto"));
                $this->idimpacto->setOldValue($this->idimpacto->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idimpacto") ?? Route("idimpacto")) !== null) {
                    $this->idimpacto->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->idimpacto->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ImpactoList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ImpactoList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ImpactoList") {
                            ...
                            $returnUrl = "ImpactoList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idimpacto' first before field var 'x_idimpacto'
        $val = $CurrentForm->hasValue("idimpacto") ? $CurrentForm->getValue("idimpacto") : $CurrentForm->getValue("x_idimpacto");
        if (!$this->idimpacto->IsDetailKey) {
            $this->idimpacto->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->idimpacto->CurrentValue = $this->idimpacto->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idimpacto->Visible && $this->idimpacto->Required) {
                if (!$this->idimpacto->IsDetailKey && EmptyValue($this->idimpacto->FormValue)) {
                    $this->idimpacto->addErrorMessage(str_replace("%s", $this->idimpacto->caption(), $this->idimpacto->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // impacto
        $this->impacto->setDbValueDef($rsnew, $this->impacto->CurrentValue, $this->impacto->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['impacto'])) { // impacto
            $this->impacto->CurrentValue = $row['impacto'];
        }

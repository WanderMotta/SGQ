 ...
 * Page class
 */
class DocumentoInternoEdit extends DocumentoInterno
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentoInternoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentoInternoEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("iddocumento_interno") ?? Key(0) ?? Route(2)) !== null) {
                $this->iddocumento_interno->setQueryStringValue($keyValue);
                $this->iddocumento_interno->setOldValue($this->iddocumento_interno->QueryStringValue);
            } elseif (Post("iddocumento_interno") !== null) {
                $this->iddocumento_interno->setFormValue(Post("iddocumento_interno"));
                $this->iddocumento_interno->setOldValue($this->iddocumento_interno->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->iddocumento_interno->IsDetailKey) {
            $this->iddocumento_interno->setFormValue($val);
        }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->iddocumento_interno->Visible && $this->iddocumento_interno->Required) {
                if (!$this->iddocumento_interno->IsDetailKey && EmptyValue($this->iddocumento_interno->FormValue)) {
                    $this->iddocumento_interno->addErrorMessage(str_replace("%s", $this->iddocumento_interno->caption(), $this->iddocumento_interno->RequiredErrorMessage));
                }
            }
        ...
        if (in_array("revisao_documento", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("RevisaoDocumentoGrid");
            if (in_array("revisao_documento", $detailTblVar) && $detailPage->DetailEdit && $editRow) {

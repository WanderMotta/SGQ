 ...
 * Page class
 */
class IndicadoresList extends Indicadores
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IndicadoresList";
    ...
    public $FormName = "findicadoreslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "IndicadoresList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "IndicadoresAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "IndicadoresDelete";
        $this->MultiUpdateUrl = "IndicadoresUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "findicadoresgrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("IndicadoresList");
                    return;
                }
            } else {
    ...
    {
        $this->meta->FormValue = ""; // Clear form value
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idindicadores->CurrentValue;
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_indicador") &&
            $CurrentForm->hasValue("o_indicador") &&
            $this->indicador->CurrentValue != $this->indicador->DefaultValue &&
            !($this->indicador->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->indicador->CurrentValue == $this->indicador->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_unidade_medida_idunidade_medida") &&
            $CurrentForm->hasValue("o_unidade_medida_idunidade_medida") &&
            $this->unidade_medida_idunidade_medida->CurrentValue != $this->unidade_medida_idunidade_medida->DefaultValue &&
            !($this->unidade_medida_idunidade_medida->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->unidade_medida_idunidade_medida->CurrentValue == $this->unidade_medida_idunidade_medida->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_meta") &&
            $CurrentForm->hasValue("o_meta") &&
            $this->meta->CurrentValue != $this->meta->DefaultValue &&
            !($this->meta->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->meta->CurrentValue == $this->meta->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->idindicadores->AdvancedSearch->toJson(), ","); // Field idindicadores
        $filterList = Concat($filterList, $this->dt_cadastro->AdvancedSearch->toJson(), ","); // Field dt_cadastro
        $filterList = Concat($filterList, $this->indicador->AdvancedSearch->toJson(), ","); // Field indicador
        ...
        $filterList = Concat($filterList, $this->unidade_medida_idunidade_medida->AdvancedSearch->toJson(), ","); // Field unidade_medida_idunidade_medida
        $filterList = Concat($filterList, $this->meta->AdvancedSearch->toJson(), ","); // Field meta
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("findicadoressrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idindicadores
        $this->idindicadores->AdvancedSearch->SearchValue = @$filter["x_idindicadores"];
        $this->idindicadores->AdvancedSearch->SearchOperator = @$filter["z_idindicadores"];
        $this->idindicadores->AdvancedSearch->SearchCondition = @$filter["v_idindicadores"];
        $this->idindicadores->AdvancedSearch->SearchValue2 = @$filter["y_idindicadores"];
        $this->idindicadores->AdvancedSearch->SearchOperator2 = @$filter["w_idindicadores"];
        $this->idindicadores->AdvancedSearch->save();
        ...
        // Field indicador
        $this->indicador->AdvancedSearch->SearchValue = @$filter["x_indicador"];
        $this->indicador->AdvancedSearch->SearchOperator = @$filter["z_indicador"];
        $this->indicador->AdvancedSearch->SearchCondition = @$filter["v_indicador"];
        $this->indicador->AdvancedSearch->SearchValue2 = @$filter["y_indicador"];
        $this->indicador->AdvancedSearch->SearchOperator2 = @$filter["w_indicador"];
        $this->indicador->AdvancedSearch->save();
        ...
        // Field unidade_medida_idunidade_medida
        $this->unidade_medida_idunidade_medida->AdvancedSearch->SearchValue = @$filter["x_unidade_medida_idunidade_medida"];
        $this->unidade_medida_idunidade_medida->AdvancedSearch->SearchOperator = @$filter["z_unidade_medida_idunidade_medida"];
        $this->unidade_medida_idunidade_medida->AdvancedSearch->SearchCondition = @$filter["v_unidade_medida_idunidade_medida"];
        $this->unidade_medida_idunidade_medida->AdvancedSearch->SearchValue2 = @$filter["y_unidade_medida_idunidade_medida"];
        $this->unidade_medida_idunidade_medida->AdvancedSearch->SearchOperator2 = @$filter["w_unidade_medida_idunidade_medida"];
        $this->unidade_medida_idunidade_medida->AdvancedSearch->save();

        // Field meta
        $this->meta->AdvancedSearch->SearchValue = @$filter["x_meta"];
        $this->meta->AdvancedSearch->SearchOperator = @$filter["z_meta"];
        $this->meta->AdvancedSearch->SearchCondition = @$filter["v_meta"];
        $this->meta->AdvancedSearch->SearchValue2 = @$filter["y_meta"];
        $this->meta->AdvancedSearch->SearchOperator2 = @$filter["w_meta"];
        $this->meta->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->idindicadores, $default, false); // idindicadores
        $this->buildSearchSql($where, $this->dt_cadastro, $default, false); // dt_cadastro
        $this->buildSearchSql($where, $this->indicador, $default, false); // indicador
        $this->buildSearchSql($where, $this->periodicidade_idperiodicidade, $default, false); // periodicidade_idperiodicidade
        $this->buildSearchSql($where, $this->unidade_medida_idunidade_medida, $default, false); // unidade_medida_idunidade_medida
        $this->buildSearchSql($where, $this->meta, $default, false); // meta
        ...
        if (!$default && $this->Command == "search") {
            $this->idindicadores->AdvancedSearch->save(); // idindicadores
            ...
            $this->unidade_medida_idunidade_medida->AdvancedSearch->save(); // unidade_medida_idunidade_medida
            $this->meta->AdvancedSearch->save(); // meta
    ...
    public function showFilterList()
    {
        ...
        // Field indicador
        $filter = $this->queryBuilderWhere("indicador");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->indicador, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->indicador->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field unidade_medida_idunidade_medida
        $filter = $this->queryBuilderWhere("unidade_medida_idunidade_medida");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->unidade_medida_idunidade_medida, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->unidade_medida_idunidade_medida->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field meta
        $filter = $this->queryBuilderWhere("meta");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->meta, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->meta->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->idindicadores->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->unidade_medida_idunidade_medida->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->meta->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->idindicadores->AdvancedSearch->unsetSession();
        ...
        $this->unidade_medida_idunidade_medida->AdvancedSearch->unsetSession();
        $this->meta->AdvancedSearch->unsetSession();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->indicador->Expression . " ASC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->indicador, $ctrl); // indicador
            $this->updateSort($this->periodicidade_idperiodicidade, $ctrl); // periodicidade_idperiodicidade
            $this->updateSort($this->unidade_medida_idunidade_medida, $ctrl); // unidade_medida_idunidade_medida
            $this->updateSort($this->meta, $ctrl); // meta
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_graficos"
        $item = &$this->ListOptions->add("detail_graficos");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'graficos');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"indicadores\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"indicadores\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"indicadores\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"findicadoreslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"findicadoreslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_graficos"
        $opt = $this->ListOptions["detail_graficos"];
        if ($Security->allowList(CurrentProjectID() . 'graficos')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("graficos", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("GraficosList?" . Config("TABLE_SHOW_MASTER") . "=indicadores&" . GetForeignKeyUrl("fk_idindicadores", $this->idindicadores->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("GraficosGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'indicadores')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=graficos");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "graficos";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'indicadores')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=graficos");
                ...
                $detailEditTblVar .= "graficos";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'indicadores')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=graficos");
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->idindicadores->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"indicadores\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_graficos");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=graficos");
        $detailPage = Container("GraficosGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'indicadores') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "graficos";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "indicador");
            $this->createColumnOption($option, "periodicidade_idperiodicidade");
            $this->createColumnOption($option, "unidade_medida_idunidade_medida");
            $this->createColumnOption($option, "meta");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"findicadoressrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"findicadoressrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="findicadoreslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"findicadoreslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"findicadoreslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_indicadores", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_indicadores",
    ...
    {
        $this->meta->DefaultValue = $this->meta->getDefault(); // PHP
        $this->meta->OldValue = $this->meta->DefaultValue;
    }
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->idindicadores->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->idindicadores->AdvancedSearch->SearchValue != "" || $this->idindicadores->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->indicador->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->indicador->AdvancedSearch->SearchValue != "" || $this->indicador->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->unidade_medida_idunidade_medida->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->unidade_medida_idunidade_medida->AdvancedSearch->SearchValue != "" || $this->unidade_medida_idunidade_medida->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->meta->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->meta->AdvancedSearch->SearchValue != "" || $this->meta->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_indicador")) {
            $this->indicador->setOldValue($CurrentForm->getValue("o_indicador"));
        }
        ...
        if ($CurrentForm->hasValue("o_unidade_medida_idunidade_medida")) {
            $this->unidade_medida_idunidade_medida->setOldValue($CurrentForm->getValue("o_unidade_medida_idunidade_medida"));
        }
        ...
        if ($CurrentForm->hasValue("o_meta")) {
            $this->meta->setOldValue($CurrentForm->getValue("o_meta"));
        }
        ...
        if (!$this->idindicadores->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType != RowType::AGGREGATEINIT && $this->RowType != RowType::AGGREGATE && $this->RowType != RowType::PREVIEWFIELD) {
            $this->indicador->Count++; // Increment count
        }
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->isExport()) {
                $this->indicador->ViewValue = $this->highlightValue($this->indicador);
            }
            ...
            $this->meta->HrefValue = "";
                ...
                $this->indicador->Count = 0; // Initialize count
        } elseif ($this->RowType == RowType::AGGREGATE) { // Aggregate row
            $this->indicador->CurrentValue = $this->indicador->Count;
            $this->indicador->ViewValue = $this->indicador->CurrentValue;
            $this->indicador->CssClass = "fw-bold";
            $this->indicador->HrefValue = ""; // Clear href value
        }
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['indicador']); // indicador
        $hash .= GetFieldHash($row['periodicidade_idperiodicidade']); // periodicidade_idperiodicidade
        $hash .= GetFieldHash($row['unidade_medida_idunidade_medida']); // unidade_medida_idunidade_medida
        $hash .= GetFieldHash($row['meta']); // meta
        return md5($hash);
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Update current values
        $this->setCurrentValues($rsnew);
        if ($this->indicador->CurrentValue != "") { // Check field with unique index
            $filter = "(`indicador` = '" . AdjustSql($this->indicador->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getAddRow()
    {
        ...
        // indicador
        $this->indicador->setDbValueDef($rsnew, $this->indicador->CurrentValue, false);

        // periodicidade_idperiodicidade
        $this->periodicidade_idperiodicidade->setDbValueDef($rsnew, $this->periodicidade_idperiodicidade->CurrentValue, false);

        // unidade_medida_idunidade_medida
        $this->unidade_medida_idunidade_medida->setDbValueDef($rsnew, $this->unidade_medida_idunidade_medida->CurrentValue, false);

        // meta
        $this->meta->setDbValueDef($rsnew, $this->meta->CurrentValue, strval($this->meta->CurrentValue) == "");
        return $rsnew;
    }
    ...
    {
        if (isset($row['indicador'])) { // indicador
            $this->indicador->setFormValue($row['indicador']);
        }
        if (isset($row['periodicidade_idperiodicidade'])) { // periodicidade_idperiodicidade
            $this->periodicidade_idperiodicidade->setFormValue($row['periodicidade_idperiodicidade']);
        }
        if (isset($row['unidade_medida_idunidade_medida'])) { // unidade_medida_idunidade_medida
            $this->unidade_medida_idunidade_medida->setFormValue($row['unidade_medida_idunidade_medida']);
        }
        if (isset($row['meta'])) { // meta
            $this->meta->setFormValue($row['meta']);
        }
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"findicadoressrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-table=\"indicadores\" data-caption=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-ew-action=\"modal\" data-url=\"IndicadoresSearch\" data-btn=\"SearchBtn\">" . $Language->phrase("AdvancedSearch", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-caption=\"" . $Language->phrase("AdvancedSearch", true) . "\" href=\"IndicadoresSearch\">" . $Language->phrase("AdvancedSearch", false) . "</a>";
        }
        $item->Visible = true;
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-table=\"indicadores\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" data-ew-action=\"modal\" data-url=\"IndicadoresQuery\" data-btn=\"SearchBtn\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" href=\"IndicadoresQuery\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        }
        $item->Visible = true;

        // Search highlight button
        $item = &$this->SearchOptions->add("searchhighlight");
        $item->Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" . $Language->phrase("Highlight") . "\" data-caption=\"" . $Language->phrase("Highlight") . "\" data-ew-action=\"highlight\" data-form=\"findicadoressrch\" data-name=\"" . $this->highlightName() . "\">" . $Language->phrase("HighlightBtn") . "</a>";
